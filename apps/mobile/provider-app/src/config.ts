...
const ENV = {
  ...
  KEYCLOAK_CLIENT_ID: process.env.EXPO_PUBLIC_KEYCLOAK_CLIENT_ID ?? "provider-app",
  API_BASE_URL: process.env.EXPO_PUBLIC_API_BASE_URL ?? "https://api.impilo.gov.zw",
  REDIRECT_URI: process.env.EXPO_PUBLIC_REDIRECT_URI ?? "impilo.provider://callback",
};

/**
 * In-memory secure storage adapter for environments without native keychain.
 * Production builds replace this with platform-native secure storage.
 */
