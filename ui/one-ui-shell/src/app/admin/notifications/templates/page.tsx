      ...
      <PageShell title="Notification templates" subtitle="Governed template CRUD via notification-service BFF">
        ...
        {isLoading ? (
          ...
          <ul className="divide-y divide-gray-100 rounded-xl border border-border bg-card">
            {(templates ?? []).length === 0 ? (
              <li className="p-4 text-sm text-muted-foreground">No templates returned from notification-service.</li>
            ) : (
              templates?.map((row) => {
                ...
                return (
                  <li key={id} className="flex flex-wrap items-center justify-between gap-3 p-4">
                    <div>
                      <p className="text-sm font-medium text-foreground">{String(row.name ?? row.key ?? id)}</p>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="rounded-lg border border-border px-3 py-1 text-xs font-medium text-muted-foreground hover:bg-background"
                      >
                        Retire
                      </button>
                    </div>
