...
describe("CommsOpsPage", () => {
  it("renders the escalation queue, adapter status and on-call roster from live endpoints", async () => {
    get.mockImplementation((path: string) => {
      if (path.endsWith("/escalations")) return Promise.resolve({ data: [{ id: "e1", priority: "URGENT", status: "OPEN", tier: 1, resolutionBreached: true }] });
      if (path.endsWith("/delivery/adapters")) return Promise.resolve({ data: [{ channel: "SMS", status: "NOT_CONFIGURED" }] });
      if (path.endsWith("/on-call")) return Promise.resolve({ data: [{ actorId: "worker-1", actorType: "PROVIDER", dutyStatus: "ON_CALL" }] });
