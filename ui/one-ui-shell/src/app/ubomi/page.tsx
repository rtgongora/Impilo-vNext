      ...
      <PageShell title="UBOMI civil registry" subtitle="Birth and death notification (CRVS) via Experience BFF" serviceSlug="ubomi">
        <div className="space-y-6">
          ...
          {!available && !statusLoading ? (
            <p className="rounded-lg border border-warning/35 bg-warning-soft p-3 text-sm text-warning-foreground">
            ...
            {(["births", "deaths", "verify"] as Tab[]).map((t) => (
              <button
                ...
                className={`px-4 py-2 text-sm font-medium capitalize ${
                  tab === t ? "border-b-2 border-teal-600 text-teal-800" : "text-muted-foreground"
                }`}
              >
                {t}
              </button>
          ...
          {tab === "births" ? (
            <section className="space-y-4">
              <div className="rounded-xl border border-border bg-card p-4">
                ...
                {!facility?.id ? (
                  <p className="text-sm text-warning-foreground">Select a facility in the header before submitting.</p>
                ) : (
                  <form
                  ...
                  >
                    ...
                    <button
                      ...
                      className="sm:col-span-2 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white hover:bg-teal-800 disabled:opacity-50"
                    >
              ...
              {birthsQ.isLoading ? (
                ...
                <ul className="divide-y divide-slate-100 rounded-xl border border-border bg-card">
                  {birthsQ.data?.length === 0 ? (
                    <li className="p-4 text-sm text-muted-foreground">No birth notifications yet.</li>
                  ) : (
              ...
              <div className="rounded-xl border border-border bg-card p-4">
                ...
                >
                  ...
                  <button
                    ...
                    className="sm:col-span-2 rounded-lg bg-primary-hover px-4 py-2 text-sm font-medium text-white hover:bg-neutral-900 disabled:opacity-50"
                  >
                ...
                {deathsQ.data?.map((row) => (
                  <li key={String(row.id)} className="flex items-center justify-between gap-3 p-4">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">#{row.notificationNumber ?? row.id} · {row.status}</p>
                    </div>
                    {row.status === "SUBMITTED" && row.id != null ? (
                      <button
                        type="button"
          ...
          {tab === "verify" ? (
            <section className="rounded-xl border border-border bg-card p-4">
              ...
              <input
                className="mt-2 w-full max-w-md rounded-lg border border-border px-3 py-2 text-sm"
                value={regNo}
                onChange={(e) => setRegNo(e.target.value)}
                placeholder="Enter registration number"
              />
              {verifyQ.isFetching ? <p className="mt-3 text-sm text-muted-foreground">Looking up…</p> : null}
              {verifyQ.isError ? (
                <p className="mt-3 text-sm text-danger">Verification lookup failed.</p>
              ) : null}
              {verifyQ.data?.data ? (
                <dl className="mt-4 grid gap-3 rounded-lg border border-border bg-background p-4 sm:grid-cols-2 text-sm">
                ...
                <p className="mt-3 text-sm text-muted-foreground">No registration found for that number.</p>
              ) : null}
            </section>
          ) : null}
        </div>
