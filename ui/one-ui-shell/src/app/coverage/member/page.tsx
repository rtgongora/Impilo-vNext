...
function Section({
  ...
  return (
    <section className="rounded-xl border border-border bg-card shadow-sm overflow-hidden">
      <div className="flex items-center gap-2 border-b border-border px-4 py-3 bg-violet-50/60">
        {icon}
  ...
  return (
    <AppLayout>
      ...
      >
        ...
        {!cpid && (
          <p className="mb-4 text-sm text-warning-foreground bg-warning-soft border border-warning/35 rounded-lg px-4 py-3">
        ...
        <div className="flex flex-col gap-6">
          ...
          >
            {plans.length === 0 ? (
              <p className="text-sm text-muted-foreground">No active coverage plans found for your profile.</p>
            ) : (
                    ...
                    {plans.map((row, i) => {
                      ...
                      return (
                        <tr key={readStr(row, "id") || i}>
                          <td className="py-2 pr-2 font-medium text-foreground">{name || "—"}</td>
                          <td className="py-2 pr-2 text-foreground">{payer || "—"}</td>
                          <td className="py-2 pr-2">
                            <span
                              className={`inline-block px-2 py-0.5 text-xs rounded-full font-medium ${
                                status === "ACTIVE" ? "bg-green-100 text-green-800" : "bg-neutral-100 text-foreground"
                              }`}
                            >
                              {status}
                            </span>
          ...
          >
            {latestEligibility ? (
              ...
              <p className="text-sm text-muted-foreground">No eligibility checks on file yet.</p>
            )}
            <div className="mt-4 space-y-2 border-t border-border pt-4">
              <p className="text-xs font-semibold text-foreground">Run a new check</p>
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                />
                <input
                  className="sm:w-40 rounded-lg border border-border px-3 py-2 text-sm"
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {claimsSlice.map((c, i) => {
                        ...
                        const badge = CLAIM_STATUS[st] ?? "bg-neutral-100 text-foreground";
                        return (
                          <tr key={id || i}>
          ...
          >
            {contributions.length === 0 ? (
              <p className="text-sm text-muted-foreground">No contribution records.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                ...
                <tbody className="divide-y divide-gray-100">
                  {contributions.map((row, i) => {
                    ...
                    const badge = CONTRIB_STATUS[st] ?? "bg-neutral-100 text-foreground";
          ...
          >
            {preauths.length === 0 ? (
              <p className="text-sm text-muted-foreground">No preauthorizations.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                ...
                <tbody className="divide-y divide-gray-100">
                  {preauths.map((row, i) => {
                    ...
                    const badge = PREAUTH_STATUS[st] ?? "bg-neutral-100 text-foreground";
                    return (
                      <tr key={readStr(row, "id") || i}>
          ...
          >
            {!utilizationQ.isLoading && !utilizationQ.isError && utilizationRows.length === 0 ? (
              <p className="text-sm text-muted-foreground">No benefit utilization tracked yet</p>
            ) : null}
            {!utilizationQ.isLoading && !utilizationQ.isError && utilizationRows.length > 0 ? (
                ...
                {utilizationRows.map((row, i) => {
                  ...
                  return (
                    <div
                      ...
                      className="rounded-lg border border-border bg-background/80 p-4 space-y-3"
                    >
                      <div>
                        <p className="text-sm font-semibold text-foreground">{benefit || "Benefit"}</p>
                        {plan ? <p className="text-xs text-muted-foreground font-mono mt-0.5">{plan}</p> : null}
                        <p className="text-xs text-muted-foreground mt-1">{period}</p>
                      </div>
                      <dl className="grid grid-cols-3 gap-2 text-xs">
                        <div>
                          <dt className="text-muted-foreground">Annual limit</dt>
                          <dd className="font-medium text-foreground">{limit > 0 ? limit.toLocaleString() : "—"}</dd>
                        </div>
                        <div>
                          <dt className="text-muted-foreground">Used</dt>
                          <dd className="font-medium text-foreground">{used.toLocaleString()}</dd>
                        </div>
                        <div>
                      ...
                      <div>
                        ...
                        {pct !== null ? (
                          <p className="text-[10px] text-muted-foreground mt-1">{pct.toFixed(0)}% of annual limit used</p>
                        ) : (
                          <p className="text-[10px] text-muted-foreground mt-1">No annual limit set for this benefit</p>
                        )}
                      </div>
                    </div>
                  );
          ...
          <Section
            title="Appeals"
            icon={<Gavel className="h-4 w-4 text-foreground" />}
          ...
          >
            ...
            {appealOpen && (
              <div className="mb-4 rounded-lg border border-border bg-background p-4 space-y-2">
                <input
            ...
            {appeals.length === 0 ? (
              <p className="text-sm text-muted-foreground">No appeals filed.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b text-left text-muted-foreground">
                ...
                <tbody className="divide-y divide-gray-100">
                  {appeals.map((row, i) => {
                    const st = readStr(row, "status") || "PENDING";
                    const badge = APPEAL_STATUS[st] ?? "bg-neutral-100 text-foreground";
                    return (
                      <tr key={readStr(row, "id") || i}>
                        <td className="py-2 pr-2 font-mono text-xs">{readStr(row, "claimId", "claim_id")}</td>
                        <td className="py-2 pr-2 text-foreground max-w-xs truncate">
                        ...
                        <td className="py-2 text-foreground">{readStr(row, "decision") || "—"}</td>
                      </tr>
                    );
                  })}
                </tbody>
