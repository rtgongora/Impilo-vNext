          ...
          <>
            <div className="mb-6 rounded-lg border border-border bg-card p-5">
              <div className="flex flex-wrap items-center gap-2 text-xs">
                {structure.category ? (
                  <span className="rounded bg-neutral-100 px-1.5 py-0.5 text-foreground">{structure.category}</span>
                ) : null}
                {structure.level ? (
                  <span className="rounded bg-neutral-100 px-1.5 py-0.5 text-foreground">{structure.level}</span>
                ) : null}
                {structure.cpdEligible ? (
                  <span className="inline-flex items-center gap-1 rounded bg-success-soft px-1.5 py-0.5 text-primary-hover">
                    <BadgeCheck className="h-3 w-3" /> CPD
                  </span>
                ) : null}
                {structure.mandatory ? (
                  <span className="inline-flex items-center gap-1 rounded bg-info-soft px-1.5 py-0.5 text-blue-800">
              ...
              {structure.description ? (
                <p className="mt-3 text-sm text-foreground">{structure.description}</p>
              ) : null}
                  ...
                  <li key={m.id} className="rounded-lg border border-border bg-card p-4">
                    ...
                    {m.lessons.length > 0 ? (
                      <ul className="mt-3 space-y-1.5 pl-3">
                        {m.lessons.map((l) => (
                          <li key={l.id} className="flex items-start gap-2 text-sm text-foreground">
                            ...
                            <span>
                              ...
                              {!l.required ? (
                                <span className="ml-2 rounded bg-neutral-100 px-1 text-[10px] uppercase text-muted-foreground">
                                  Optional
                                </span>
                              ) : null}
                            </span>
                      ...
                      <p className="mt-2 text-xs italic text-muted-foreground">No lessons yet.</p>
                    )}
                  </li>
                ))}
              </ol>
