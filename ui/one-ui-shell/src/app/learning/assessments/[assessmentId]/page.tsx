      ...
      <PageShell title={String(assessment.title ?? "Assessment")} subtitle="Assessment detail and question preview.">
        <div className="mb-3 flex items-center justify-between gap-4 rounded border border-border bg-card p-3">
          <div className="text-sm">
            ...
            <p className="text-xs text-muted-foreground">{canAttempt ? "You can start a new attempt." : "Maximum attempts reached."}</p>
          </div>
          {canAttempt ? (
        ...
        <ul className="space-y-2">
          {questions.map((q) => (
            <li key={String(q.id)} className="rounded border border-border bg-card p-3 text-sm">
              <p className="font-medium text-foreground">{String(q.prompt)}</p>
              <p className="text-xs text-muted-foreground">{String(q.type)}</p>
            </li>
          ))}
        </ul>
        {attempts.length > 0 ? (
