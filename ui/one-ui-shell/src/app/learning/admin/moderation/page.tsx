        ...
        <ul className="space-y-2" data-testid="fundo-moderation-queue">
          {items.map((assessment) => (
            <li key={String(assessment.id)} className="rounded border border-gray-200 bg-white p-3 text-sm">
              <p className="font-medium text-gray-900">{String(assessment.title ?? assessment.id)}</p>
              <p className="text-xs text-gray-500">{String(assessment.assessmentType ?? "ASSESSMENT")}</p>
              <Link
