      ...
      <PageShell title="Edit course" subtitle="Update course metadata and publish/archive status.">
        ...
        <div className="mt-4 grid gap-4 lg:grid-cols-2">
          <div className="rounded border border-border bg-card p-4">
            <p className="text-sm font-medium text-foreground">Add module</p>
            <input className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" placeholder="Module title" value={moduleTitle} onChange={(e) => setModuleTitle(e.target.value)} />
            <button onClick={addModule} className="mt-2 rounded border border-border px-3 py-1.5 text-sm text-foreground">Add module</button>
          ...
          <div className="rounded border border-border bg-card p-4">
            <p className="text-sm font-medium text-foreground">Add lesson</p>
            <select className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" value={moduleIdForLesson} onChange={(e) => setModuleIdForLesson(e.target.value)}>
              <option value="">Select module</option>
              {modules.map((m) => (
                <option key={String(m.id)} value={String(m.id)}>{String(m.title ?? m.id)}</option>
              ))}
            </select>
            <input className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" placeholder="Lesson title" value={lessonTitle} onChange={(e) => setLessonTitle(e.target.value)} />
            <select className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" value={lessonContentType} onChange={(e) => setLessonContentType(e.target.value)}>
            ...
            <select className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" value={lessonContentFormat} onChange={(e) => setLessonContentFormat(e.target.value)}>
              <option value="PLAIN_TEXT">PLAIN_TEXT</option>
              <option value="MARKDOWN">MARKDOWN</option>
              <option value="STRUCTURED_BLOCKS">STRUCTURED_BLOCKS</option>
            </select>
            <textarea className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" placeholder="Lesson text content" value={lessonBody} onChange={(e) => setLessonBody(e.target.value)} />
            <input className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" placeholder="Reference URL (video/document/link)" value={lessonContentRef} onChange={(e) => setLessonContentRef(e.target.value)} />
            <textarea className="mt-2 w-full rounded border border-border px-2 py-1 text-sm" placeholder="Structured blocks JSON" value={lessonBlocksText} onChange={(e) => setLessonBlocksText(e.target.value)} />
            <button onClick={addLesson} className="mt-2 rounded border border-border px-3 py-1.5 text-sm text-foreground">Add lesson</button>
            <ul className="mt-3 space-y-1 text-xs text-muted-foreground">
