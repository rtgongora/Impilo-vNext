    ...
    >
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
          <p className="text-sm font-semibold text-gray-900">Add module</p>
          <input value={moduleTitle} onChange={(e) => setModuleTitle(e.target.value)} className="w-full rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Module title" />
        ...
        <div className="space-y-3 rounded-lg border border-gray-200 bg-white p-4">
          <p className="text-sm font-semibold text-gray-900">Add lesson</p>
          <select value={selectedModuleId} onChange={(e) => setSelectedModuleId(e.target.value)} className="w-full rounded border border-gray-300 px-3 py-2 text-sm">
            <option value="">Select module</option>
            {moduleOptions.map((m) => (
              <option key={m.id} value={m.id}>{m.title}</option>
            ))}
          </select>
          <input value={lessonTitle} onChange={(e) => setLessonTitle(e.target.value)} className="w-full rounded border border-gray-300 px-3 py-2 text-sm" placeholder="Lesson title" />
          <textarea value={contentBlocksText} onChange={(e) => setContentBlocksText(e.target.value)} className="h-40 w-full rounded border border-gray-300 px-3 py-2 text-xs" />
      ...
      <div className="mt-4 rounded-lg border border-gray-200 bg-white p-4">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-sm font-semibold text-gray-900">Current structure</p>
          <div className="flex gap-2 text-xs">
            <button
              type="button"
              onClick={() => setPreviewMode("CREATOR")}
              className={`rounded border px-2 py-1 ${previewMode === "CREATOR" ? "border-teal-600 bg-teal-50 text-teal-700" : "border-gray-300 text-gray-700"}`}
            >
              Creator preview
            </button>
            <button
              ...
              className={`rounded border px-2 py-1 ${previewMode === "LEARNER" ? "border-teal-600 bg-teal-50 text-teal-700" : "border-gray-300 text-gray-700"}`}
            >
              Learner preview
            </button>
          </div>
        ...
        <ul className="mt-2 space-y-2 text-sm text-gray-700">
          {modules.map((m) => (
            <li key={m.id}>
              ...
              <p className="text-xs text-gray-500">{m.lessons?.length ?? 0} lessons · {previewMode === "LEARNER" ? "learner" : "author"} view</p>
              {Array.isArray(m.lessons) && m.lessons.length > 0 ? (
                <ol className="mt-1 list-decimal pl-5 text-xs text-gray-600">
