  ...
  return (
    <FundoStudioWorkspace title="Studio Surveys" subtitle="Author survey/feedback/reflection instruments and bind to course delivery.">
      <div className="rounded border border-border bg-card p-4">
        <select value={formType} onChange={(e) => setFormType(e.target.value as "SURVEY" | "FEEDBACK" | "REFLECTION")} className="mb-2 w-full rounded border border-border px-3 py-2 text-sm">
      ...
      <div className="mt-3 rounded border border-border bg-card p-4">
        <p className="text-sm font-semibold text-foreground">Existing forms</p>
        <ul className="mt-2 space-y-2 text-sm">
          {items.map((item) => (
            <li key={item.id} className="rounded border border-border px-3 py-2">
