  ...
  return (
    <FundoStudioWorkspace title="Studio Library" subtitle="Upload and curate reusable resources for courses, lessons, assessments and AI source material.">
      <div className="rounded-lg border border-border bg-card p-4">
        <p className="text-sm font-semibold text-foreground">Add library resource metadata</p>
        <div className="mt-2 grid gap-2 md:grid-cols-3">
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Resource title" className="rounded border border-border px-3 py-2 text-sm" />
          <select value={resourceType} onChange={(e) => setResourceType(e.target.value)} className="rounded border border-border px-3 py-2 text-sm">
