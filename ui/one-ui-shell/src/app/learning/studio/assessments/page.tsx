  ...
  return (
    <FundoStudioWorkspace title="Studio Assessments" subtitle="Pre-tests, post-tests, quizzes, final assessments and practical/case reviews.">
      <div className="mb-3 rounded border border-gray-200 bg-white p-4">
        <p className="text-sm font-semibold text-gray-900">Create assessment draft</p>
        <div className="mt-2 grid gap-2 md:grid-cols-3">
          <select value={courseId} onChange={(e) => setCourseId(e.target.value)} className="rounded border border-gray-300 px-3 py-2 text-sm">
            <option value="">Select course</option>
            {items.map((c) => (
              <option key={c.id} value={c.id}>{c.title}</option>
            ))}
          </select>
          <select value={assessmentType} onChange={(e) => setAssessmentType(e.target.value)} className="rounded border border-gray-300 px-3 py-2 text-sm">
            {["PRE_TEST", "POST_TEST", "QUIZ", "FINAL_ASSESSMENT", "SURVEY", "FEEDBACK", "PRACTICAL_CHECKLIST", "CASE_REVIEW"].map((type) => (
              <option key={type} value={type}>{type}</option>
            ))}
          </select>
          <input value={title} onChange={(e) => setTitle(e.target.value)} className="rounded border border-gray-300 px-3 py-2 text-sm" />
        </div>
        ...
        {items.map((c) => (
          <li key={c.id} className="rounded border border-gray-200 bg-white p-3 text-sm">
            ...
            <div className="mt-2 flex gap-2 text-xs">
              <Link href={`/learning/admin/assessments/new?courseId=${encodeURIComponent(c.id)}`} className="rounded border border-gray-300 px-2 py-1 text-gray-700">Create assessment</Link>
              <Link href={`/learning/studio/courses/${c.id}/builder`} className="rounded border border-gray-300 px-2 py-1 text-gray-700">Attach in builder</Link>
            </div>
          </li>
        ))}
      </ul>
