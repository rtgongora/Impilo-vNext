    ...
    <FundoStudioWorkspace title="Studio AI" subtitle="Provider-agnostic AI authoring with safety metadata and draft-only persistence.">
      <div className="rounded-lg border border-border bg-card p-4">
        <div className="mb-2 grid gap-2 md:grid-cols-2">
          <select value={generationType} onChange={(e) => setGenerationType(e.target.value)} className="rounded border border-border px-3 py-2 text-sm">
            {["course-outline", "lesson-content", "quiz", "survey", "summary", "facilitator-guide", "voiceover-script"].map((kind) => (
              <option key={kind} value={kind}>{kind}</option>
            ))}
          </select>
          <select value={audience} onChange={(e) => setAudience(e.target.value)} className="rounded border border-border px-3 py-2 text-sm">
            {["NURSE", "DOCTOR", "COMMUNITY_HEALTH_WORKER", "REGISTRY_OFFICER", "SUPERVISOR"].map((role) => (
              <option key={role} value={role}>{role}</option>
            ))}
          </select>
        </div>
        <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} className="h-28 w-full rounded border border-border px-3 py-2 text-sm" />
        <div className="mt-2 flex flex-wrap gap-2">
          <button onClick={onGenerateOutline} className="rounded bg-teal-600 px-3 py-2 text-sm text-white">Generate draft</button>
          <button onClick={onNompiloHelp} className="rounded border border-border px-3 py-2 text-sm text-foreground">Ask Nompilo</button>
        </div>
        <div className="mt-3 rounded border border-warning/35 bg-warning-soft px-3 py-2 text-xs text-warning-foreground">
        ...
        {result ? <pre className="mt-3 max-h-72 overflow-auto rounded bg-background p-3 text-xs text-foreground">{result}</pre> : null}
      </div>
