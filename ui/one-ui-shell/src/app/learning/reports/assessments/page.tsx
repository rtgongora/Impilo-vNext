  ...
  return (
      <PageShell title="Assessment performance" subtitle="Attempt volumes, pass rate and scoring indicators by assessment.">
        <div className="mb-3 grid gap-2 rounded border border-border bg-card p-3 sm:grid-cols-3">
          ...
          <input value={subjectType} onChange={(e) => setSubjectType(e.target.value)} placeholder="Subject type" className="rounded border border-border px-2 py-1 text-sm" />
          <input value={assessmentId} onChange={(e) => setAssessmentId(e.target.value)} placeholder="Assessment ID (manual review queue)" className="rounded border border-border px-2 py-1 text-sm" />
        </div>
        <div className="overflow-auto rounded border border-border bg-card">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {items.map((item) => (
                <tr key={String(item.assessmentId)} className="border-t border-border">
              ...
              {items.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-3 py-6 text-center text-muted-foreground">No assessment analytics for current filters.</td>
                </tr>
              ) : null}
            </tbody>
          </table>
        ...
        {assessmentId ? (
          <div className="mt-4 rounded border border-border bg-card p-4">
            <p className="text-sm font-medium text-foreground">Manual review queue</p>
            <p className="mt-1 text-xs text-muted-foreground">Pending non-objective attempts requiring rubric-based review.</p>
            <ul className="mt-2 space-y-1 text-sm">
              {pendingItems.map((item) => (
                <li key={String(item.id)} className="flex items-center justify-between border-b border-border py-1">
                  <span>Attempt {String(item.attemptNo ?? "-")} • {String(item.subjectType ?? "-")}/{String(item.subjectId ?? "-")}</span>
                  <span className="text-xs text-warning-foreground">{String(item.manualReviewStatus ?? "PENDING")}</span>
                </li>
              ))}
              {pendingItems.length === 0 ? <li className="text-muted-foreground">No pending manual reviews.</li> : null}
            </ul>
          </div>
        ) : null}
      </PageShell>
