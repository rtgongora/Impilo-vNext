      ...
      <PageShell title="Transfusion Episode" subtitle={`Episode ${episodeId}`} icon={<Activity className="h-6 w-6" />}>
        <section className="rounded-2xl border border-gray-200 bg-white p-5 mb-4 space-y-3">
          ...
          <button
            ...
            disabled={!obsValue || observation.isPending}
        ...
        <section className="rounded-2xl border border-gray-200 bg-white p-5 space-y-3">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => complete.mutate({ outcome_status: outcome })} disabled={complete.isPending} className="rounded-xl bg-rose-600 px-3 py-2 text-sm text-white disabled:opacity-50">Complete</button>
