      ...
      <PageShell title="Order Blood" subtitle="Clinical blood product request" icon={<ClipboardList className="h-6 w-6" />}>
        <form onSubmit={handleSubmit} className="max-w-lg space-y-4 rounded-2xl border border-gray-200 bg-white p-6">
          <label className="block text-sm font-medium text-gray-700">
            Patient CPID
            <input required value={patientCpid} onChange={(e) => setPatientCpid(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm font-mono" placeholder="Patient canonical ID" />
          </label>
          <div className="grid grid-cols-2 gap-3">
            <label className="block text-sm font-medium text-gray-700">
              Blood group
              <select value={bloodGroup} onChange={(e) => setBloodGroup(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm bg-white">
                {GROUPS.map((g) => <option key={g} value={g}>{g}</option>)}
              </select>
            </label>
            <label className="block text-sm font-medium text-gray-700">
              Component
              <select value={componentType} onChange={(e) => setComponentType(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm bg-white">
          ...
          <label className="block text-sm font-medium text-gray-700">
            Units requested
            <input type="number" min={1} value={units} onChange={(e) => setUnits(e.target.value)} className="mt-1 w-full rounded-xl border border-gray-300 px-3 py-2 text-sm" />
          </label>
