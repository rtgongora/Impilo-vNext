 ...
 * Route: /operations/facility-operations/resources
 */
  ...
  return (
    <AppLayout>
      <PageShell
        ...
        subtitle="Tuso facility resources exposed through the Experience appointments faÃ§ade. Empty rows mean Tuso returned no resources or the facility identifier is not in the Tuso numeric format."
      >
        <div className="mb-4">
          <Link
        ...
        {!fid ? (
          ...
          <div className="rounded-xl border border-border bg-background p-8 text-sm text-muted-foreground">
            ...
            facility keys, the BFF returns an empty list â€” coordinate registry alignment or use a Tuso-matched facility
            id.
          </div>
        ) : (
            ...
            <table className="min-w-[640px] w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {rows.map((row, idx) => {
                  ...
                  const type = String(row.type ?? row.resource_type ?? row.resourceType ?? "â€”");
                  return (
                    <tr key={`${id}-${idx}`} className="hover:bg-background/80">
