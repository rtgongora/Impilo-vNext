...
function ModeBadge({ mode }: { mode: string }) {
  const styles: Record<string, string> = {
    ONLINE: "bg-success-soft text-primary-hover border-success/25",
    OFFLINE: "bg-warning-soft text-warning-foreground border-warning/35",
    MAINTENANCE: "bg-danger-soft text-rose-800 border-danger/28",
  };
  return (
    <span
      className={`inline-flex items-center rounded-full border px-3 py-1 text-sm font-semibold ${styles[mode] ?? "bg-background text-foreground border-border"}`}
    >
      {mode}
    </span>
  );
...
function IssueProvisionalForm({ onDone }: { onDone: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3 rounded-2xl border border-border bg-card p-5">
      <h3 className="text-sm font-semibold text-foreground">Issue provisional ID</h3>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      ...
      >
        <h3 className="font-semibold text-foreground">
          Reconcile <span className="font-mono text-primary-hover">{provisional.reference}</span>
        </h3>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 rounded-xl bg-primary-hover px-4 py-2 text-sm font-medium text-white hover:bg-impilo-700 disabled:opacity-50"
          >
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex-1 rounded-xl border border-border px-4 py-2 text-sm font-medium text-foreground hover:border-gray-400"
          >
            Cancel
          </button>
        </div>
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-foreground">OpenCR match trigger</h3>
      </div>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-2 rounded-xl border border-primary/25 bg-primary-soft px-3 py-1.5 text-sm font-medium text-impilo-900 hover:border-impilo-300"
              >
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {provisionalList.map((p) => (
                      <tr key={p.reference} className="py-2">
                        <td className="py-2 pr-4 font-mono text-xs text-foreground">{p.reference}</td>
                        <td className="py-2 pr-4 text-foreground">
                          {p.demographics.givenName} {p.demographics.familyName}
                        </td>
                        <td className="py-2 pr-4 text-muted-foreground">{p.demographics.dateOfBirth}</td>
                        ...
                        <td className="py-2 pr-4 text-xs text-muted-foreground">{p.facilityId}</td>
                        <td className="py-2">
                          {p.status === "PENDING" && (
                            <button
                              type="button"
                              onClick={() => setReconcileTarget(p)}
                              className="rounded-lg border border-border px-3 py-1 text-xs font-medium text-foreground hover:border-gray-400"
                            >
                              Reconcile
                            </button>
                          )}
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {dedupList.map((c) => (
                      <tr key={c.caseId} className="py-2">
                        <td className="py-2 pr-4 font-mono text-xs text-foreground">{c.caseId.slice(0, 8)}</td>
                        <td className="py-2 pr-4 font-mono text-xs text-foreground">{c.sourceHealthId.slice(0, 10)}</td>
                        <td className="py-2 pr-4 font-mono text-xs text-foreground">{c.targetHealthId.slice(0, 10)}</td>
                        <td className="py-2 pr-4 text-foreground">{c.score.toFixed(2)}</td>
                        <td className="py-2 pr-4">
                          <span
                            className={`rounded-full px-2 py-0.5 text-xs font-medium ${
                              c.status === "RESOLVED"
                          ...
                          {c.status === "OPEN" && (
                            <div className="flex gap-2">
                              ...
                              <button
                                ...
                                className="rounded-lg border border-border px-3 py-1 text-xs font-medium text-foreground hover:border-gray-400 disabled:opacity-50"
                              >
                                Ignore
                              </button>
                            </div>
