...
describe("provider-registry-onboarding golden thread", () => {
  ...
  it("UI route wires bounded-context client", () => {
    ...
    expect(page).toContain("useRegistry");
  });
  ...
  it("BFF controller exposes journey endpoints", () => {
    const controller = readFileSync(resolve(repoRoot, "services/experience-bff/src/main/java/zw/gov/mohcc/impilo/experience/controller/RegistryController.java"), "utf8");
    expect(controller).toContain("/internal/v1/registry");
  });
});
