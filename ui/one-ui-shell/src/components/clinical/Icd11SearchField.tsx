  ...
  return (
    <div className="space-y-1">
      ...
      <div className="relative">
        ...
        {results.length > 0 && (
          <ul className="absolute z-10 mt-1 w-full max-h-48 overflow-y-auto rounded border bg-card shadow divide-y">
            {results.map((r) => (
              <li key={`${r.code}-${r.description}`}>
                <button
                  ...
                  className="block w-full px-2 py-2 text-left text-sm hover:bg-danger-soft"
                >
                  <span className="font-mono text-xs text-muted-foreground">{r.code}</span>
                  <span className="ml-2">{r.description}</span>
                  {r.chapter && <span className="block text-xs text-muted-foreground">{r.chapter}</span>}
                </button>
              </li>
            ))}
          </ul>
