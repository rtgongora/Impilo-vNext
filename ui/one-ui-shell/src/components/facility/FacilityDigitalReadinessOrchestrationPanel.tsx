...
export function FacilityDigitalReadinessOrchestrationPanel() {
  ...
  return (
    <section
      className="rounded-2xl border border-success/25 bg-success-soft/50 p-4"
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-xs text-primary-hover/80" data-testid="facility-readiness-kpi">
            {!facility
      ...
      {facility && rows.length > 0 && (
        <ul className="mt-3 grid gap-2 sm:grid-cols-2 text-xs text-emerald-950">
          {rows.map((row) => (
            <li key={row.id} className="rounded-lg border border-emerald-100 bg-card/80 px-3 py-2">
