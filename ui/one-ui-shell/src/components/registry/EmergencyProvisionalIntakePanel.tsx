...
const inputClass =
  "w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-rose-500";
...
export function EmergencyProvisionalIntakePanel() {
  ...
  return (
    <section className="rounded-2xl border border-rose-200 bg-rose-50/50 p-5" data-testid="emergency-provisional-intake-panel">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <button
          ...
          className="rounded-lg border border-rose-300 bg-white px-3 py-1.5 text-xs font-medium text-rose-900 hover:bg-rose-100 disabled:opacity-50"
        >
      ...
      <form onSubmit={handleSubmit} className="mt-4 grid gap-3 md:grid-cols-2">
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Provisional first name</label>
          <input value={form.firstName} onChange={(e) => setForm((c) => ({ ...c, firstName: e.target.value }))} className={inputClass} />
        </div>
        <div>
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Provisional last name</label>
          <input value={form.lastName} onChange={(e) => setForm((c) => ({ ...c, lastName: e.target.value }))} className={inputClass} />
        </div>
        <div className="md:col-span-2">
          <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500">Triage / arrival notes</label>
