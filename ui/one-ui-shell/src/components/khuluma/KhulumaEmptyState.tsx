...
export interface KhulumaAction {
  label: string;
  href?: string;
  onClick?: () => void;
...
/**
 * The Khuluma empty-state standard. A dead "No channels match this search" is never
 * acceptable — every empty surface must explain the feature, say when it fills, offer at
 * least one working action, and optionally hand off to Nompilo.
 */
export function KhulumaEmptyState({
  icon,
  title,
  explanation,
  when,
  ...
  return (
    <div className="rounded-lg border border-dashed border-border bg-muted/30 px-4 py-6 text-center">
      ...
      {guidance ? (
        <p className="mx-auto mt-4 flex max-w-md items-start gap-1.5 rounded-lg bg-emerald-50 px-3 py-2 text-left text-xs text-emerald-800">
