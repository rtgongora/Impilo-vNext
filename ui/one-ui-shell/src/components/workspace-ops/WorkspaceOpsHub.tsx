...
const WORKSPACE_TYPE_META: Record<WorkspaceOpsType, { icon: React.ComponentType<{ className?: string }>; color: string; label: string }> = {
  clinical: { icon: Stethoscope, color: 'bg-primary', label: 'Clinical Workspace' },
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex items-center gap-3 mb-4 shrink-0">
        {onBack && (
          <button
            onClick={onBack}
            className="h-8 w-8 flex items-center justify-center rounded-md hover:bg-neutral-100 transition-colors"
          >
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="px-2 py-0.5 rounded border border-border text-[10px] text-muted-foreground shrink-0">
              {meta.label}
            </span>
          </div>
          {facilityName && (
