  ...
  return (
    <AdministrationGovernanceShell title={steps.title} subtitle={option.outcome} requireGovernanceEntry={false}>
      <div className="space-y-6">
        <Link href="/work/administration-governance/onboard" className="inline-flex items-center gap-1 text-sm text-slate-600 hover:text-slate-900">
        ...
        {!allowed ? (
          ...
          <>
            <div className="rounded-xl border border-slate-200 bg-white p-5">
              ...
              <p className="mt-2 text-sm font-medium text-emerald-800">Outcome: {option.outcome}</p>
            </div>

            <ol className="space-y-3">
              {steps.steps.map((step, index) => (
                <li key={step} className="flex gap-3 rounded-xl border border-slate-200 bg-white p-4">
                  <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-sm font-semibold text-indigo-800">
                    {index + 1}
                  </span>
                  <div>
                    <p className="font-medium text-slate-900">{step}</p>
                    <p className="mt-1 text-xs text-slate-500">Guided step — BFF workforce-governance APIs apply OPA precheck on submit.</p>
                  </div>
                </li>
              ))}
            </ol>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Link href="/work/administration-governance/access-requests" className="rounded-lg border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
