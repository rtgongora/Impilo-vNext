/**
 * Inventory Web -- Trust-Aware API Client
 *
 * Every outbound request injects the mandatory trust headers from the
 * ops session store. This ensures that Envoy ext_authz -> TSHEPO
 * can evaluate every request before it reaches the Inventory backend.
 *
...
async function request<T>(
  ...
  const headers: Record<string, string> = {
    ...
    "X-Purpose-Of-Use": state.purposeOfUse || "OPERATIONS",
    "X-Device-Fingerprint": "inventory-ops-web",
  };
