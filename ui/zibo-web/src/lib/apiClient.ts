/**
 * ZIBO Web -- Trust-Aware API Client
 *
 * Every outbound request injects the mandatory trust headers from the
 * admin session store. This ensures that Envoy ext_authz -> TSHEPO
 * can evaluate every request before it reaches the ZIBO backend.
 *
...
async function request<T>(
  ...
  const headers: Record<string, string> = {
    ...
    "X-Device-Fingerprint": "zibo-admin-web",
  };
