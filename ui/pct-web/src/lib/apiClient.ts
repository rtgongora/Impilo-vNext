/**
 * PCT Web -- Trust-Aware API Client
 *
 ...
 * can evaluate every request before it reaches the PCT backend.
 *
...
async function request<T>(
  ...
  const headers: Record<string, string> = {
    ...
    "X-Purpose-Of-Use": state.purposeOfUse || "TREATMENT",
  };
  ...
  if (state.shiftId) {
    headers["X-Shift-Id"] = state.shiftId;
  }
