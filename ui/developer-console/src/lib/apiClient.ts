/**
 * Developer Console — Trust-Aware API Client
 *
 * Every outbound request injects the mandatory trust headers from the
 * developer's session store. This ensures that Envoy ext_authz → TSHEPO
...
async function request<T>(
  ...
  const { session, workContext, purposeOfUse } = useDeveloperSession.getState();
