      ...
      >
        ...
        <div className="max-w-5xl space-y-6">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="text-sm font-semibold text-slate-900">Claims list</h2>
            ...
            {claimsQ.isLoading ? (
              ...
              <p className="mt-3 text-sm text-red-700">Could not load payer claims.</p>
            ) : claims.length > 0 ? (
              <div className="mt-4 overflow-x-auto rounded-xl border border-slate-200">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-left">
                    <tr>
                      <th className="px-3 py-2 font-medium text-slate-700">Claim</th>
                      <th className="px-3 py-2 font-medium text-slate-700">Bill</th>
                      <th className="px-3 py-2 font-medium text-slate-700">Insurer</th>
                      <th className="px-3 py-2 font-medium text-slate-700">Status</th>
                      <th className="px-3 py-2 font-medium text-slate-700">Submitted</th>
                      <th className="px-3 py-2 font-medium text-slate-700">Open</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {claims.map((claim) => (
                      <tr key={claim.claimId}>
                        <td className="px-3 py-3 font-mono text-xs text-slate-700">{claim.claimId}</td>
                        <td className="px-3 py-3 text-slate-700">{claim.billId ?? "—"}</td>
                        <td className="px-3 py-3 font-mono text-xs text-slate-700">{claim.insurerId ?? "—"}</td>
                        <td className="px-3 py-3 text-slate-700">{claim.status ?? "—"}</td>
                        <td className="px-3 py-3 text-slate-700">{claim.submittedAt ?? "Not submitted"}</td>
              ...
              <p className="mt-3 text-sm text-slate-500">No payer claims matched the current filters.</p>
            )}
          </section>
        </div>
      </PageShell>
