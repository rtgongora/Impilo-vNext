              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Link
                      href={`/ehr/${patientId}/preferences/communications`}
                    ...
                    <span
                      className="rounded-lg border border-amber-200/80 bg-white/80 px-3 py-1.5 text-xs text-amber-950"
                      title="Capture and withdrawals are handled in the Mvumo consent registry (operator shell)."
                    >
                      Request / update in Mvumo registry
                    </span>
                  </div>
                </div>
                ...
                {patientSummary?.consentSummary && (
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    ...
                    <div className="rounded-md border border-amber-100 bg-white/90 p-3 md:col-span-2">
                      <p className="text-[10px] font-semibold uppercase tracking-wide text-slate-500">
                        Communication preference profile (Mvumo)
                      </p>
                      {patientSummary.consentSummary.communicationPreferenceProfile?.hasProfile ? (
                        <p className="mt-2 text-sm text-slate-800">
                          State:{" "}
                          <span className="font-medium">
                            {patientSummary.consentSummary.communicationPreferenceProfile.lifecycleState ?? "—"}
                          </span>
                          {" · "}
                          Bundle v{patientSummary.consentSummary.communicationPreferenceProfile.bundleVersion ?? "—"}
                          {patientSummary.consentSummary.communicationPreferenceProfile.updatedAt ? (
                            <>
                              {" · "}
                              Updated {patientSummary.consentSummary.communicationPreferenceProfile.updatedAt}
                            </>
                          ) : null}
                        </p>
                      ) : (
                        <p className="mt-2 text-sm text-slate-600">
                          No versioned communication preference bundle on file yet — preferences are not one-time
                          registration fields; they can be added or revised in intake, portal, or assisted workflows.
                        </p>
                      )}
                    </div>
                    <div className="md:col-span-2 rounded-md border border-slate-200 bg-slate-50/80 p-3 text-xs text-slate-600">
                      Communication / data / blood restrictions are listed in the structured Mvumo response; workbench
                      workflows (orders, theatre, comms) should read the same BFF field for context-sensitive alerts.
                    </div>
                  </div>
                )}
              </div>
