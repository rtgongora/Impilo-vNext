    ...
    <div>
      ...
      {loading ? (
        ...
        <table className="w-full bg-white rounded shadow text-sm">
          ...
          <tbody>
            {catalogs.map(c => (
              <tr key={c.catalogId} className="border-t hover:bg-gray-50">
                ...
                <td className="px-4 py-3 space-x-2">
                  {c.status === 'DRAFT' && (
                    <button onClick={() => action(c.catalogId, 'submit-review')}
                      className="text-xs text-blue-600 hover:underline">Submit Review</button>
                  )}
                  {c.status === 'REVIEW' && (
                  ...
                  <a href={`/items?catalogId=${c.catalogId}`}
                    className="text-xs text-gray-600 hover:underline">View Items</a>
                </td>
              </tr>
            ))}
          </tbody>
