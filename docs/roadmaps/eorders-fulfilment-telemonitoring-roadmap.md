...
| **M1 — Trust spine live** | Signed, versioned, token-claimable prescriptions + order lifecycle on the estate | Wave OF-A deployed + journeys #50/#51 legs green live | 🔶 IN PROGRESS (built + local rig 36/36; estate rollout running) |
| **M2 — Marketplace live** | RFO round → offer → commitment with real stock reservation, fail-closed eligibility | Journeys #41 (to reservation), #48, #49, #52 green live | 🔶 IN PROGRESS (built + suite-green; live proof pending) |
| **M3 — Money + dispense closed** | Coverage/liability/payment wired into selection; dispense bound to claims; pickup/delivery | Wave OF-B; journeys #42–#47, #53–#57, #68–#70 | ⬜ NEXT |
...
## Wave OF-A — P0 foundations (CURRENT)

**Built + test-green + committed (2026-07-23):**
...
**Remaining to close M1+M2 (in flight):**
1. Estate rollout of the 8 touched services (running).
2. Provision `PRESCRIPTION_SIGNING` key for the golden tenant (`POST /v1/keys/provision`).
3. Live proofs: trust-spine smoke, then marketplace legs #41/#48/#49/#52 against real VARAPI/TUSO/VASHANDI + seeded vendor/stock.
4. Matrix/backlog evidence-tag updates + wave-close commit.
...
## Wave OF-B — P1 marketplace-complete + finance (M3)
