...
| **E** | Event catalog + AsyncAPI or schema table for Kafka surfaces | Parallel **per bounded context** (e.g. clinical vs finance outbox) | **Slices 1–2** (2026-04-12): catalog + [`pct-clinical-encounter.asyncapi.yaml`](../../contracts/asyncapi/pct-clinical-encounter.asyncapi.yaml); PCT/Costa/BFF topic + payload alignment; **+1** cycle for broader AsyncAPI + remaining topic audit |
...
**Phase E (in progress):** **Slice 1** — [`kafka-event-catalog.md`](../architecture/kafka-event-catalog.md) + AsyncAPI workspace README. **Slice 2 (done)** — Reconciled **PCT encounter** topics: `CostaEventConsumer` + BFF `UpstreamEventConsumer` use `pct.encounter.started` / `completed`; [`EncounterService`](../../services/pct-service/src/main/java/zw/gov/mohcc/impilo/pct/core/EncounterService.java) payloads add **`eventId`** + **`tenantId`** (+ **`patientCpid`** on complete) for Costa; **surveillance** BFF listener uses **`impilo.surv.case.opened.v1`**; removed orphan `surv.outbreak.declared` subscription; DR/bootstrap docs + [`pct-clinical-encounter.asyncapi.yaml`](../../contracts/asyncapi/pct-clinical-encounter.asyncapi.yaml). **Next slice:** expand AsyncAPI / catalog to finance rails (`mushex.*`, `costa.*`) and OROS pharmacy fan-out.
