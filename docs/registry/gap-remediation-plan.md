...
| Wellness taxonomy drift (`simba-service` vs `wellness-service`) | ownership confusion across clinical/experience boundaries | **remediated in service inventory reconciliation pass:** `simba-service` and `wellness-service` reclassified to explicit clinical wellness domains with doctrine-aligned SoR/forbidden maps; retain as governance watch item for route parity evidence |
