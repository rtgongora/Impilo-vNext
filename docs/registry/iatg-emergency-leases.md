...
|---|---|---|---|---|
| `pct-service` | **V500** | adult medicine V100–V129 · surgery V300–V329 · paeds/IMAM V400–V429 · RMNP V430–V459 · telemedicine V500–V529 · trauma V035–V069 · problem-list V060 (exception, §3c) | **V200–V239** | ✅landed 2026-07-28: episode **V200**, medical-episode FK **V201**, triage IITT **V202**, alerts **V203**, handover **V204**, diagnostic-order lifecycle **V205**, order-set instance/item **V206** (+ `EmergencyEpisodeController` — the episode/handshake HTTP surface had ZERO callers anywhere for 3 waves, found and fixed while starting W7b), emergency medication admin w/ authoriser≠witness two-person guard **V207** (W8b), episode-level disposition (15 types, R12 mandatory-content gate) + observation/short-stay (one-open-per-episode) **V208**, handover `response_due_at` + `HandoverExpirySweepJob` — the auto-expiry V204's own header deferred as "a later scheduled job" **V209** (both W9b), and **W5b (no new migration)** — `EmergencyAlertController`/`EmergencyAlertService` (ack/respond/close on the existing V203 alerts, previously unreachable over HTTP), `EmergencyReconciliationJob` (the CORRELATION_DIVERGENCE reconciler both V203's own CHECK vocabulary and `EmergencyAlertRules`'s class comment cited but nothing performed — polls DAIDZAI's `GET /trauma-episodes/{id}` on a slower 5-min schedule, kept separate from the 60s alert sweep on purpose so a degraded peer only slows reconciliation), and `RitoSafetyClient` (mirrors inpatient-theatre's client exactly — same `/internal/v1/rito/cases` contract) wired synchronously into both `EmergencyAlertSweepJob` (ACCEPTANCE_OVERDUE opens a case at raise time) and `HandoverExpirySweepJob` (every expiry opens a case, closing V204's own deferred promise), and **W6b (no new pct migration)** — `InFacilityDeteriorationConsumer` consumes inpatient's `EMERGENCY_ACTIVATION_RAISED`, resolves the admission's EXISTING journey via `pct_admissions.inpatient_admission_ref` (V018, `AdmissionRepository.findByTenantIdAndInpatientAdmissionRef`, added this wave), mints an `emergency_episode` on it via `entry_route=IN_FACILITY_DETERIORATION` (idempotent on `entrySourceRef=activationId`), then calls the new `InpatientActivationLinkClient` to write `emergency_episode_id` back onto the activation — the round-trip V200's own correlation column named but nothing completed until now. An activation with no `admissionRef`, or one with no matching PCT admission handshake yet, is skipped, never guessed at, and **W10 (no new pct migration)** — `GET /v1/emergency/command-summary?facilityId=` composes `EmergencyEpisodeService.board` + `EmergencyAlertService.board` into episode-by-state and alert-by-severity counts; no new pct SoR, per this wave's own scope (capacity lives in tuso), and **W11 (no new pct migration)** — `MciBulkMintService` (`POST /v1/emergency/mci/{incidentId}/bulk-mint?facilityId=`): pulls daidzai's unminted MCI casualties, mints one `emergency_episode` per casualty via the existing idempotent `EmergencyEpisodeService.open()` (`entrySourceService=daidzai-service`, `entrySourceRef=casualtyId`), `entry_route=MASS_CASUALTY`, starts `PRE_ARRIVAL` with no journey (reuses the existing `/arrive` endpoint for anchor resolution rather than a parallel state machine), then calls back `DaidzaiEpisodeClient.linkMciCasualtyEpisode` to write `emergency_episode_id` onto the casualty row — the same write-back round-trip pattern as W6b's `InpatientActivationLinkClient`. Unreachable daidzai mints nothing that pass, never guesses, and **V210 landed 2026-07-28 (W12)** — `emergency_identity_link`: append-only clinical identity-resolution audit for an emergency episode (resolution_method closed to BIOMETRIC/DOCUMENT/RELATIVE/SMART_CARD/CLINICAL_RECOGNITION, evidence_ref, optional vito_merge_id, superseded_by for corrections), `EmergencyIdentityLinkService.linkIdentity` (`POST /v1/emergency/episodes/{id}/identity-link`) plus history read, `EMERGENCY_IDENTITY_LINKED` routed off the catch-all. **Not** a second copy of every VITO repoint — `PctEmergencyRepointHook` is unchanged and still handles the async bulk-repoint path alone; this table is the WHY behind a clinical resolution, populated only by that workflow. `superseded_by`'s FK is `DEFERRABLE INITIALLY DEFERRED` (real-Postgres verification caught that a plain immediate FK and the partial `uq_eil_active_per_episode` unique index cannot both be satisfied mid-correction — the app supersedes the old row then inserts the new one, in one transaction, deferred-checked at commit). Also this wave: `scripts/guard/check-identity-repoint-coverage.sh` (scoped to pct/inpatient/daidzai/madi, the wired trauma lane — NOT repo-wide; costa and rito-quality-safety carry `patient_cpid`/`subject_cpid` with zero hook wiring today, pre-existing debt outside this pack's remit) proven both ways and registered in `run-change-safety-gates.sh`; it found **3 real gaps in daidzai's OWN prior waves** — `dai_mci_casualty.subject_cpid` (this pack's own W11), `dai_emergency_request.subject_cpid`, `dai_assistance_request.subject_cpid` — all closed by the new `DaidzaiIntakeRepointHook` (daidzai repo methods only, no daidzai migration needed — the columns already existed). **Correction to this file's own W6's plan quote**: the design doc's claim that `emergency_identity_link` works "exactly as COSTA already does" is false — verified costa has no `IdentityRepointHook` implementation, no identity-link table, and no `resolution_method`/`evidence_ref` columns anywhere; there was no existing pattern to port, this is new, and **V211 landed 2026-07-28 (W15)** — the episode-spine reachability fix: `ed_visit.emergency_episode_id` (nullable, `DEFERRABLE`-free plain FK to `emergency_episode`). **Critical finding**: nothing that registers an ED walk-in/ambulance arrival — `EdVisitService.openVisit`, the actual ED registration path and the dominant real-world emergency entry route — ever called `EmergencyEpisodeService.open()`. Only `InFacilityDeteriorationConsumer` (W6b) and `MciBulkMintService` (W11) minted episodes; the ENTIRE spine (six alerts, 15-disposition R12 gate, acceptance handshake, command view, identity-link audit) was invisible for ordinary ED care — the third instance this pack has found of its own "backend built, never wired" failure class (after W0's header drop and W10's unreachable controller), found and closed in the same wave it surfaced in. `openVisit` now mints the episode synchronously (journeyId already known at registration, so CC-5-anchored from creation, no PRE_ARRIVAL window) and stamps it back; idempotent on `(entry_route, entrySourceRef=visit_id)`. `recordDisposition` now ALSO closes the real 15-type episode disposition from the same ed_visit disposition write (DISCHARGE→DISCHARGED_HOME, ADMIT→ADMITTED_WARD, TRANSFER→TRANSFERRED_OUT, REFER→REFERRED_OUTPATIENT, LAMA→LEFT_AGAINST_ADVICE, LWBS→LEFT_BEFORE_ASSESSMENT, DEATH→DIED), best-effort: the episode's mandatory-content gate (R12) is stricter than ed_visit's own form ever collected, so a mapping missing a required field (e.g. `ADMIT` with no destination) is logged and skipped rather than fabricating a value to force the gate — the episode stays honestly open rather than closing on invented data. `LEFT_BEFORE_ASSESSMENT`/`LEFT_AGAINST_ADVICE` supply `lastSeenAt=now()` — the recording moment IS a real last-observed fact for a patient who has left, not a placeholder. Verified: real-Postgres migration + FK/index, and a new 8-case unit suite (`EdVisitServiceEmergencyEpisodeTest`) — the ONLY test coverage `EdVisitService` has that surefire actually runs; its sole other test, `EdVisitIT`, is a `*IT.java` excluded from the build. pct-service 825/825 green, and **W15b (no new migration)** — MCI casualty tagging UI, deliberately deferred from W11 per that wave's own notes ("scoped OUT building a full daidzai-facing BFF client... treated as a deliberate boundary"): `DaidzaiServiceClient`/`DaidzaiController` gain `tagCasualty`/`listCasualties`/`unmintedCasualties`/`updateCasualtyStatus` proxying daidzai's existing `MciCasualtyController` (W11), and a new `/work/daidzai/disasters/[id]/casualties` page (extending the existing disaster-command surface rather than forking a new one under `/clinical/emergency/**`) — sieve-tag a casualty, re-rate status, and trigger PCT's `mci/{incidentId}/bulk-mint` (already BFF-proxied since W11) with the current facility id. Linked from the disasters list for `incidentType=MCI` rows only. Verified: tsc clean, 4/4 vitest, route-parity/no-stub/query-honesty/launcher guards all clean for this wave's files · next: board **V212–13** · reserve V214–39 |
| `madi-service` | V200 | trauma V015–V044 (MTP sub-range **superseded**, §3b) | **V200–V229** | ✅landed 2026-07-28 (W8a): MHP activation + pack tracking **V200** — reuses `BloodOrderService.emergencyRelease`'s existing `EmergencyAccessGuard` break-glass gate (single-order O-neg release already existed; the protocol-level multi-unit activation did not — proven not a duplicate before writing). Reserve V201–29. |
| `inpatient-service` | **V300** (surgery P4) | surgery V067–V080 **and V300–V329** · adult medicine V111–V130 | **V200–V229** | ✅landed 2026-07-28 (W6a): tenant backfill on `resuscitation_record`/`_phase` (brownfield, backfilled from `emergency_activation`, verified on real Postgres with a pre-seeded legacy row) + `resuscitation_medication` (new, idempotent-retry from creation, correct-never-delete) + `emergency_activation.origin`/`emergency_episode_id` + `protocol_type` CHECK (`NOT VALID` — that column has been fully unconstrained free text since V007) — all in **V200**, one migration. Deliberately did NOT add `subject_cpid` to record/phase: the identity anchor stays ONLY on `emergency_activation` per `InpatientResuscitationRepointHook`'s own design; duplicating it would be a drift bug, not a fix. **✅landed 2026-07-28 (W6b): V201** — `resuscitation_event.client_event_id` (idempotent-retry) + `resuscitation_record` header hardening (`team_leader`/`current_rhythm`/`airway_status`/`access_status`/`outcome`/`stand_down_reason`) + `record_version` (JPA `@Version`, header-only optimistic lock — the time-series stays append-only/never locked). `cpr_cycles`/`defibrillations`/`rosc_achieved`/`initial_rhythm` are DERIVED from `resuscitation_event` since this wave (never client-set again — `InpatientClinicalService.recomputeResuscitationSummary`); `final_rhythm` stamps only at the stand-down moment (first `outcome` supplied). Batch endpoint (`/resuscitation/events/batch`) pre-checks `client_event_id`s (both against the DB and within the same batch) rather than catching the DB's own constraint violation — a real Postgres transaction aborts on the first constraint failure inside it, so catch-and-continue per item in one `@Transactional` batch would have poisoned every insert after the first duplicate. `activateEmergency` now sets `origin` and emits `EMERGENCY_ACTIVATION_RAISED` (never emitted before this wave) on a new `inpatient.emergency` legacy topic + the auto-derived v1.1 topic; a new `POST /emergency/{id}/link-episode` lets PCT write `emergency_episode_id` back after minting. Reserve V202–29. **⚠ Surgery's V300 sits ABOVE this band — see the DDL rule below.** |
...
| `daidzai-service` | **V202** | trauma V010–V049 | **V200–V229** | ✅landed: generalise + PCT back-link **V200**, defer anchor enforcement **V201** · merge/adopt needed no migration (status is free VARCHAR) · **V202 landed 2026-07-28 (W11)**: `dai_mci_casualty` (MCI sieve tags — IMMEDIATE/URGENT/DELAYED/EXPECTANT/DECEASED, deliberately distinct from R7's single-patient WHO IITT), `surge_declared_at`/`surge_casualty_count` on `dai_emergency_incident` (stamped once by `MciCasualtyService.SURGE_THRESHOLD=10`, an ENGINEERING_SEED not a ratified MoHCC threshold), `idx_dai_mci_casualty_unminted` partial index for PCT's bulk-mint pull. Verified on real Postgres: all 3 CHECKs + the composite unique + the incident FK + the partial index probed both ways · **W12 (no new daidzai migration)**: `DaidzaiIntakeRepointHook` — `repointSubjectCpid` JPQL added to `MciCasualtyRepository`/`EmergencyRequestRepository`/`AssistanceRequestRepository` (columns already existed, just had no repoint path — the exact gap `check-identity-repoint-coverage.sh` was written to catch) · reserve V203–29 |
