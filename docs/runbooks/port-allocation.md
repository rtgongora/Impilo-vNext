...
| **8395** | `procedures-service` | Clinical Procedures Pipeline platform layer: canonical procedure catalogue, appropriateness/duplication, competence and privilege resolution, readiness engine, safety-pause templates, aftercare templates, procedure execution index, pipeline analytics. Engine-not-store: evaluates, never persists another service's record. Does **not** own the procedure request (oros-service) or the execution record (inpatient-service). See [ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES](../architecture/adr/ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES.md) |
| **8396** | `surgery-service` | Surgery and Surgical Specialties domain pack: a management-course record that attaches to PCT journeys and never contains them (episode, structured assessment, operative indication, prehabilitation/optimisation, planned-vs-performed reconciliation, complication pathway instances, waiting-list revalidation) + 15 specialty extensions. Condition/diagnosis/staging/recurrence/surveillance/outcomes are PCT's (`pct_problems`/`pct_care_plans`, CC-2 amendment) — surgery contributes, never duplicates. PCT-anchored per CC-5; references `inpatient.procedure_episode` per operation; never a second theatre workflow. See [ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES](../architecture/adr/ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES.md) §5/§5a |
| **8397** | `mental-health-service` | **LANDED 2026-07-28 (W13)** — Integrated Emergency pack (product-owner ruling 2026-07-26: build a real service, not a typed refusal). The accepting counterparty for a psychiatric emergency handover: `mh_referral` (idempotent on handover_id, PENDING→ACCEPTED/DECLINED, writes back to pct's `emergency_handover` via `PctHandoverClient`), `mh_assessment` (capacity-outcome consistency enforced), `mh_risk_formulation`, `mh_safety_plan`, `mh_involuntary_episode` (statutory detention with a free-text legal-basis field — no fabricated Zimbabwe Mental Health Act picklist — and at-most-one-OPEN-per-referral), `mh_restraint_event` (restraint-*reduction* register: mandatory de-escalation-attempted record, review queue), `mh_admission_request` (raises to PCT, never decides — `pct_admission_id` set only via `acknowledge`), `mh_followup`. Care Continuum **component**, `continuum_parent: pct-service`; must not own the admission decision, person-level longitudinal registries, or a second care continuum. Claimed in [iatg-emergency-leases](../registry/iatg-emergency-leases.md) §4 — originally planned as 8395, moved after the surgery lane took 8395/8396 mid-flight. 8398 remains free; 8399 is `referral-service`. |
