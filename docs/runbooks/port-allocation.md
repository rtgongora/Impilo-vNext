...
| **8396** | `surgery-service` | **RESERVED** — Surgery and Surgical Specialties domain pack: surgical disease system of record (episode, condition, staging, indication, decision, prehabilitation, complication pathways, histology closure, outcomes, surveillance, recurrence) + 15 specialty extensions. PCT-anchored per CC-5; references `inpatient.procedure_episode` per operation; never a second theatre workflow. See [ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES](../architecture/adr/ADR-SURGERY-AND-PROCEDURES-SERVICE-BOUNDARIES.md) |
