...
| OF-B8 | Ruvimbo coverage and prior authorisation — **PARTIAL** (PA half live-proven 2026-07-23; medicine-registry/formulary half BUILT Wave OF-D 2026-07-23, awaiting deploy + live proof) | P1 | No |
...
> **Status 2026-07-23:** honestly **PARTIAL** @ `46a6634b1`. Landed: the PA half — `requires_authorisation`/PA flag on liability estimates (coverage V019), PA_REQUIRED gates commitment (§8.7.6) and a minimum-necessary PA is auto-submitted onto live `cv_authorisations` (SUBMITTED) — live #47 (fixrun3; PA idempotency key made body-exact @ `9fcb42f2f`, auth-type vocabulary corrected to coverage-owned `PRIOR`).
>
> **Wave OF-D 2026-07-23 (formulary half BUILT, not yet deployed/live-proven):** ZIBO national medicine registry landed as a **CodeSystem, not a new artifact type** (design decision — medicines are concepts+properties of the existing CODE_SYSTEM machinery; the ordinal-persisted `ArtifactType` enum is untouched and now ordinal-guard-tested): zibo V007 registers `http://www.whocc.no/atc` with a 20-medicine EDLIZ starter set (genericName/form/strength/scheduleClass properties, 2 controlled) so validate-coding resolves ATC (the OF-B3 finding), plus `/v1/medicines` resolve/search over the same artifact. Coverage V020 `cv_formulary` (plan-version-keyed like `cv_benefit_definitions`; medication_code+coding_system → **benefit_code mapping**, covered flag, tier/copay_class, requires_authorisation, effective window) + `FormularyService`/`FormularyController` (lookup + governed CRUD, delete=window-retire) wired into `LiabilityEstimateService`: medication estimates resolve through the formulary (formulary PA flag surfaces exactly like the benefit-level flag; excluded/not-listed/expired price honestly non-covered). msika-flow `OfferFinancialsService` interim replaced: ATC-coded lines (published-snapshot codingSystem) pass the ATC code as `medicationCode` via `CoverageClient.estimateMedicationLiability` (disjoint idempotency-key family) and the PA flow uses the MAPPED benefit code; non-medication lines unchanged. Golden-tenant seed: 11 formulary rows per governed plan (2 PA on Private Plus, 1 explicit exclusion). Suites: zibo 39/0, coverage 71/0, msika-flow 287/0 (5 skips). Deferrals: full EDLIZ/MCAZ import = governed data programme; deploy + live proof; OF-G14 closes only on live proof.
