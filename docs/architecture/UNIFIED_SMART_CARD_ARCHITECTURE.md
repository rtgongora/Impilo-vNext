...
| **3. Virtual card + financial offline purse + social funding** | ✅ **BUILT** | The "needs SE hardware" blocker was wrong — the card exists in **VIRTUAL form first** (a wallet pass on the phone, software P-256 key), and the crypto is identical. VITO: `card_form` VIRTUAL\|PHYSICAL + device public key propagated on card events (payload now carries `publicKey`/`cardForm`). mushe: caches the key (`cards.vito_card_public_key`), `OfflineTransactionService` verifies a **card-signed offline txn** (dependency-free JDK SHA256withECDSA over the canonical payload), enforces a **monotonic counter** (replay/gap), and reconciles via an **idempotent ledger debit** (nonce = idempotency key) — backs tap-to-pay / scan-to-pay done offline. Plus **shareable "help pay my bill" contribution links** (`BillContributionService` — a person/group chips in; each contribution idempotently credits the beneficiary wallet, auto-fulfils at target). Unit-tested with real software P-256 keys. *Live end-to-end needs a deploy-time check with a real device key.* |
...
**Net:** every facet of the unified card is now built and tested — the *unification core* (canonical credential + lifecycle events + revoke→freeze safety loop), the *full Phase-3 financial vision* (virtual card, offline purse, tap/scan-to-pay mechanism, social bill-contribution links), and the *Phase-4 PHR facet* (fail-closed per-card consent gate + working envelope encryption). The only remaining item is an operational hardening, not a functional gap: graduate the fail-closed master key to a rotating platform data-key/KMS service when one exists. Operator prerequisite from §9: migrate any legacy vito card-wallet balances into mushe before dropping the deprecated vito ledger.
