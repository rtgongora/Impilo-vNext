...
**Net:** every facet of the unified card is now built and tested — the *unification core* (canonical credential + lifecycle events + revoke→freeze safety loop), the *full Phase-3 financial vision* (virtual card, offline purse, all three pay methods — tap/scan/biometric — plus social bill-contribution links), and the *Phase-4 PHR facet* (fail-closed per-card consent gate + working envelope encryption). This closes the SMART-card unification epic. §10b tracks the post-backend items: the mobile UI, the ops wallet-drain runbook, and the KMS custody seam are now built too — the only genuinely-open item is deploy-time integration proof, which needs a running estate.

## 10b. Closeout & handoff

Of the four post-backend items, **three are now built**; only the one that genuinely needs a running estate remains open.

1. **Client (mobile/UI) — the card's face and taps. ✅ BUILT (code-complete, not runtime-proven).** The citizen app now has the SMART card in its personal tab (`apps/mobile/citizen-app/src/screens/personal/SmartCardSection.tsx`): the **virtual card render** (front with the three functions + flip to a **real QR**), a fail-closed **PHR-carry toggle**, **biometric scan-to-pay**, and a shareable **"help pay my bill"** link. Real device crypto (`services/smartCardKey.ts`): a P-256 keypair via `@noble/curves` in the device keychain (`expo-secure-store`) signs the canonical `{amount,counter,nonce,currency,authMethod}` payload (SHA256withECDSA, DER) exactly to the server contract; the biometric gate is `expo-local-authentication`; the public key is emitted as X.509 SPKI PEM for VITO enrollment. Reaches the backend through a new **BFF card plane** (`experience-bff` `CitizenCardController` under `/internal/v1/wallet/**`) that proxies to mushe. TypeScript typechecks clean against existing types (only the newly-declared deps resolve on `pnpm install`). *Not built/run here — needs `pnpm install` + a dev-client/EAS build. **Device-key enrollment to VITO** is the one remaining wire before a live pay succeeds; until then biometric pay fails closed (signature won't match an unprovisioned key). **NFC HCE** tap-to-pay stays Android-only — iOS forbids host-card-emulation for payments; the QR and biometric paths cover both platforms.*
2. **Deploy-time integration checks (genuinely needs a running estate — still open).** (a) Live cross-service Kafka: a VITO `CARD_REVOKED` actually freezes the linked mushe money card end-to-end. (b) A real device-key offline transaction verifies and reconciles. Both are green in unit tests with software keys; they can only be proven over the wire on a deployed estate. This is verification, not code.
3. **Ops prerequisite — legacy wallet drain. ✅ BUILT (runbook + detection SQL).** [`docs/runbooks/legacy-vito-wallet-drain.md`](../runbooks/legacy-vito-wallet-drain.md) + [`.sql`](../runbooks/legacy-vito-wallet-drain.sql): a resumable, idempotent (`vito-drain:<walletId>`) reconciliation that credits mushe → verifies → zeroes VITO, with the one real correctness risk called out (VITO keys by `health_id` UUID, mushe by CPID — resolve, never assume equality), an unreconciled-entry guard, and a DROP gate on the reconciliation total reaching zero. Deliberately **not** a Flyway migration. *Running it needs live data + an operator.*
4. **KMS hardening. ✅ BUILT (seam); the KMS service itself is a future platform capability.** Key custody is now behind `CardDataKeyProvider` (`services/mushe-wallet-service/.../card/`). The default `MasterKeyCardDataKeyProvider` keeps the fail-closed HMAC-of-master-key behaviour and is `@ConditionalOnMissingBean`, so a future KMS-backed provider bean takes precedence with **no other code change**. PHR-on-card encryption already works today; only the managed rotating key service remains a platform decision.
