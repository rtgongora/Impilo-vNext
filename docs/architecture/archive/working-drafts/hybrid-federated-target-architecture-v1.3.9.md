...
>
> **v1.3.9 went to independent review on 2026-08-05 and freeze was refused** on two stop conditions. It is superseded by **v1.3.10**.
>
> **STOP-1 — the amendment contradicted its own ADR.** ADR-0055 decision 1 names `controller_type`, `data_controller_legal_name` and `data_controller_contact` as becoming nullable and non-authoritative. v1.3.9 corrected two of the three and left `controller_type VARCHAR(32) NOT NULL`, so creating a trust domain still forced declaring a controlling-arrangement type — the same fabrication defect the amendment exists to remove, one field over.
>
> **STOP-2 — four required controls had no guard.** Declaring descriptive metadata a resolution fallback, mapping automatically from organisation type, treating `UNMAPPED` as blank, and treating the projection `trust_domain_id` as authoritative all passed the verifier green.
>
> **Active document:** [`../../hybrid-federated-target-architecture-v1.3.10.md`](../../hybrid-federated-target-architecture-v1.3.10.md)
>
> Kept for provenance only. Do not implement from this file. *(Banner added by v1.3.10.)*
