...
>
> **This file is not the architecture.** v1.3.5's freeze review (2026-08-05) **refused freeze** on two stop conditions. It was never approved and never architecture-frozen, and is superseded by **v1.3.6**.
>
> **STOP-1** — §38C's entailment check parsed the register row on `|`, so a pipe inside the quoted phrase truncated what the machine verified while a human read the whole cell. The v1.3.4 mis-citation (outcome 8 → A66) could be restored with the verifier green. *The mechanism built to stop machine and reader seeing different things could itself be made to show them different things.*
> **STOP-2** — §38A.2 journey 14 named **A43** ("air-gapped install") as exact evidence for *"commissioning refuses out of order"*, which **A38** proves.
>
> **Active document:** [`../../hybrid-federated-target-architecture-v1.3.6.md`](../../hybrid-federated-target-architecture-v1.3.6.md) · entry point: [`../../ARCHITECTURE_PRECEDENCE.md`](../../ARCHITECTURE_PRECEDENCE.md)
>
> Kept for provenance only. Do not copy its §38C parser or its range citations. *(Banner added by v1.3.6.)*
