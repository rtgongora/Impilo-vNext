...
> # 📌 SUPERSEDED — BUT THIS VERSION WAS GENUINELY FROZEN
>
> **v1.3.8 was approved and architecture-frozen by the Product Owner on 2026-08-05 under ADR-0054.** It is the first version in this lineage that was ever frozen, and unlike v1.3.1–v1.3.7 it was never refused. It is archived here because it was superseded by a narrowly scoped technical correction, not because it failed.
>
> **Superseded by v1.3.9** under **ADR-0055**, which corrects one technical inconsistency: §3.2 required mandatory controller identity on `trust_domain`, `organisation.trust_domain_id NOT NULL`, and a blanket backfill of every existing organisation to `MOHCC-ZW` — while §3.1 and §3A correctly hold that a trust domain is not a controller and that controllership resolves through `processing_role_assignment`. The repository contains private-provider and local-authority organisations, so the backfill would have fabricated governance facts before determination.
>
> **ADR-0054 required this to be a new version rather than an erratum**: it changes schemas and technical design, both of which that ADR names substantive.
>
> **Active document:** [`../../hybrid-federated-target-architecture-v1.3.9.md`](../../hybrid-federated-target-architecture-v1.3.9.md)
>
> Cite this file only for what the baseline said between 2026-08-05 and the v1.3.9 correction. *(Banner added by v1.3.9.)*
