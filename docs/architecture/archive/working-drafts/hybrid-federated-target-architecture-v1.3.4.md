...
>
> **This file is not the architecture.** v1.3.4's independent freeze review (2026-08-05) **refused freeze** on three stop conditions. It was never approved and never architecture-frozen, and is superseded by **v1.3.5**.
>
> **STOP-A** — §38B outcome 8 cited A66 (a *data-bearing support session* refused when the audit sink is unreachable) where the exact criterion is **A8** (*"a break-glass that cannot be audited does not proceed"*).
> **STOP-B** — §38B outcome 5 cited A87 (*state is a vector*) where the exact criterion is **A1** (*forged `X-Tenant-ID`/`X-Facility-ID`/`X-Provider-ID` have zero effect*).
> **STOP-C** — §38A.3 marked journeys 2 and 6 `PARTIALLY_PROVEN` on the strength of A75, a specified-only invariant with no implementation, one clause after asserting that no journey is represented as proven on a written criterion.
>
> §38B was written to close the mis-citation defect class and reproduced it internally, twice. v1.3.5 adds the entailment register (§38C) that makes the class machine-checkable.
>
> **Active document:** [`../../hybrid-federated-target-architecture-v1.3.5.md`](../../hybrid-federated-target-architecture-v1.3.5.md) · entry point: [`../../ARCHITECTURE_PRECEDENCE.md`](../../ARCHITECTURE_PRECEDENCE.md)
>
> Kept for provenance only. Do not implement from this file and do not copy its §38A totals or its §38B criterion citations. *(Banner added by v1.3.5.)*
