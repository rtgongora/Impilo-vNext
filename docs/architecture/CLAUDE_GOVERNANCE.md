...
2. Read `hybrid-federated-target-architecture-v1.3.7.md` as the latest working architecture — the versioned file is the complete active document (`vnext-hybrid-federation-target-architecture.md` is only a pointer to it). It is not frozen. Obey its implementation gate.
...
6. Check the v1.3.7 implementation gate before touching code.
