...
| CJ14 | Add a child / dependant | ⚠️ BROKEN LIVE | backend+UI+unit/render-tests all green, but the live zero-to-one harness caught a **production failure**: BFF `CitizenDependantController` → `vito.registerIdentity` (child) is **denied 403 at VITO's trust boundary** (rejected before the controller — carries the citizen context, not a trusted service identity) → 502 to the citizen. Fix: the BFF child-register sub-call needs a trusted service identity / an authorised path (not the forwarded citizen bearer). Found by `citizen-journeys.sh` RUN_MUTATING (`a4dc5c31b`); every unit + render test passed. `[J]` |
