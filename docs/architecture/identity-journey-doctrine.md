...
| CJ11 | Book appointment + self-check-in | ✅ Done | full stack: `AppointmentCheckinQr` (real scannable QR via qrcode.react) on the appointment page → BFF `CitizenAppointmentCheckinController` signed scoped-token (scope-bound + replay-proof) → `/checkin-by-token` → `AppointmentCheckInService` (`4cbea9823`, envelope-fix `6c06d6764`, UI+QR `1d40c6cf9`) `[H]` (facility kiosk scanner surface still to build; not yet live-driven) |
