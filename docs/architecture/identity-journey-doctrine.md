...
| CJ11 | Book appointment + self-check-in | ✅ live-verified | full stack (`AppointmentCheckinQr` QR → BFF signed scoped-token → `AppointmentCheckInService`, `4cbea9823`/`6c06d6764`/`1d40c6cf9`). Live-driving first found the token 500 (scoped-token signing broken estate-wide: 3 stacked infra/contract breaks — keys URL unset, keys RestTemplate had no trust-header interceptor, and `/v1/sign` contract mismatch whose unit test had stubbed the wrong shape). Base session fixed all three (`d5bd4002c` +2); **now PASSES live** — `citizen-journeys.sh` RUN_MUTATING issues a valid check-in token end-to-end. `[H]` |
...
| CJ14 | Add a child / dependant | ✅ LIVE-VERIFIED | backend+UI+unit/render-tests all green AND the live zero-to-one harness now passes end-to-end (fresh person registers a real child + guardianship). The live break was **misdiagnosed at first** as a 403 on `vito.registerIdentity`; driving it with logs proved the child register **succeeds** and the 403 was on the *next* hop, `mvumo.createDelegation` — mvumo **correctly** forbids a self-service `CITIZEN` from self-asserting a `GUARDIANSHIP` delegation (the self-grant IDOR guard in `DelegationService.create`). Fix (not a weakening of that guard): in the guardian-adds-dependant flow the BFF **is** the administrative authority — it authenticated the guardian and **minted the child's brand-new health id itself**, so `delegatorSubjectRef` can never be a client-supplied foreign record (IDOR closed by construction). The BFF now presents a `SYSTEM` actor context on that one call (`MvumoServiceClient.createDelegation`), with the human guardian still recorded as `delegateActorId` for accountability; the shared interceptor's actor-header forwarding became only-if-absent so the deliberate override survives (`ServiceClientConfig.forwardHeaderIfAbsent`). Proven by `citizen-journeys.sh` RUN_MUTATING → PASS=12/FAIL=0. `[J]` |
...
**Client-journey roll-up (2026-07-19, after live-driving):** the sweep counted 13/20 "satisfied," but **live-driving demoted that number** — the point of §3's completion criteria. Honest live status of this program's six, after driving each as a freshly-registered person: **CJ13 ✅ live-verified** (own seeded consent flows back, enveloped), **CJ8 ✅ live-verified** (report-lost path), **CJ3 ✅ live-verified** (claim/start live + uniform anti-enum), **CJ4 ✅ live-verified** (recovery/start live), **CJ14 ✅ live-verified** (add-dependant — the live 403 was on `mvumo.createDelegation`, not VITO register; mvumo's self-grant IDOR guard correctly rejects a CITIZEN self-asserting guardianship, so the BFF now vouches `SYSTEM` for the child it minted itself), **CJ11 ✅ live-verified** (scoped-token signing fixed — the `localhost:8184` red herring was a stale pod; real bug was tshepo-identity `/v1/sign` contract drift, now round-trips). Distinct root causes; the boundary is NOT uniformly broken (CJ3/CJ4 internal calls always succeeded). Lesson restated: unit-green + render-green + even *screen-built* did not mean done — only registering a fresh person and driving the journey as them proved which ones actually work. **7 Built** (CJ1, CJ6, CJ7, CJ12, CJ15, CJ19, CJ20) predate this program. **7 Reuse** need wiring: CJ2 (private-match landed; registration UX), CJ5 (lost-phone), CJ9/CJ10 (biometric — gated on `X1` ABIS), CJ16 (demographic risk-tiers), CJ17 (dedup/merge steward), CJ18 (foreign national). The ✅ Done + several Reuse still need their citizen-facing UI page (UI lane) and gateway-auth acceptance-pack proof before SOFTWARE_CONTRACT_GREEN (§8).
