...
| CJ11 | Book appointment + self-check-in | ⚠️ BROKEN LIVE | full stack built (`AppointmentCheckinQr` QR → BFF signed scoped-token → `AppointmentCheckInService`, `4cbea9823`/`6c06d6764`/`1d40c6cf9`), but live-driving found the token 500s: **tshepo-identity signs scoped tokens via `http://localhost:8184/v1/sign` → Connection refused** — the keys-service URL is `localhost`, not the in-cluster `tshepo-keys-service`, so ALL scoped-token signing is broken on the estate (deploy-config bug, not just CJ11). Found by `citizen-journeys.sh` (`81f63b520`). `[H]` |
