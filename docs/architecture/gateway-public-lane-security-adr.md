...
| `/internal/v1/public/facility-certificates/credential-scan` (POST) | tuso `FacilityCredentialController` via `PublicCertificateVerificationBffController` | W-UI (Provider+Place) | facility QR credential scan (FJ QR/D-L7); signed QR in the body (never the URL); returns the policy-filtered public projection (name/code/type/location/public status) or a uniform `NOT_RECOGNISED` shape — forged/revoked/missing-facility all indistinguishable (no oracle); read-only verification, no state change; distinct from the HPA certificate verify |
...
| `/internal/v1/public/gateway/practitioners/badge-scan` (POST) | varapi `PublicBadgeVerificationController` via `PublicGatewayPractitionerBffController` | W-UI (Provider+Place) | provider badge QR scan (PJ6/D-P8); **anonymous READ-only** (verification, no state change); signed QR in the body (never the URL/logs); forged/revoked/not-in-standing all return the same `NOT_RECOGNISED` shape (no oracle); strictly separate from login and the claim journey |
...
| `/internal/v1/public/gateway/coverage/**` (GET) | coverage `PublicPlanCatalogController` via `PublicGatewayCoverageBffController` | PF-W4 | Plan/benefit catalogue comparison (care-plane) — plan definitions + plan-version benefits, allow-listed (no member/eligibility/claims; BenefitPositionResponse never mapped). Membership/eligibility/claims require sign-in. Rides GET gateway/** permitAll |
| `/internal/v1/public/gateway/wellness/**` | simba `PublicWellnessController` via `PublicGatewayWellnessBffController` | PF-W5 | GET screening-programme definitions (care-plane); **POST /wellness/calculate** is a COMPUTE-ONLY, STATELESS calculator (BMI/BP) that PERSISTS NOTHING — an anonymous-write-but-stateless lane (doctrine §13.3): no repository/entity touched, inputs validated, no data retained. Tracking requires sign-in. GET rides gateway/** permitAll; POST /calculate registered as an anonymous-write allowance |
| `/internal/v1/public/gateway/learning/**` (GET) | learning `PublicCourseCatalogController` via `PublicGatewayLearningBffController` | PF-W5 | PUBLISHED course catalogue browse (care-plane) — catalogue facts only (no /structure, /assessments, enrolment, progress). Enrolment/certificates/CPD require sign-in. Rides GET gateway/** permitAll |
| `/internal/v1/public/gateway/marketplace/**` (GET) | msika `PublicListingBrowseController` + msika-flow `PublicVendorController` via `PublicGatewayMarketplaceBffController` | PF-W3 | Public marketplace browse — PUBLISHED listings search/detail (msika) + approved-vendor directory (msika-flow, care-plane). Allow-listed catalogue facts (title/summary/risk/indicative price/fulfilment/media/verification badge); no seller PII, no orders. Ordering/paying/prescriptions require sign-in; basket held client-side (PD-7). Rides GET gateway/** permitAll. (product-registry `PublicProductController` built service-side; BFF wiring deferred — needs a base-url client) |
| `/internal/v1/public/gateway/regulatory/**` (GET) | tuso `PublicRegulatoryRequirementsController` + varapi `PublicCouncilController` via `PublicGatewayRegulatoryBffController` | PF-W2 | Reference-only "what you need to apply": facility licensing application-types/classifications/fees/rules (tuso, tenant-agnostic catalogues) + professional councils & registers (varapi, registry-plane tenant). Allow-listed fields (no internal ids, source/provenance, validation, approver, owner, PII); starting an application requires sign-in. Rides `GET /internal/v1/public/gateway/**` permitAll |
| `/internal/v1/public/gateway/blood/drives` (GET) | madi `PublicDonationDriveController` via `PublicGatewayBloodBffController` | PF-W2 | Current blood-donation appeals (PUBLISHED/OPEN drives, care-plane); allow-listed facts (title/description/type/location/jurisdiction/window/capacity) — no donor PII; registering as a donor requires sign-in. Rides the GET gateway/** permitAll |
| `/internal/v1/public/gateway/notices` (GET) | guidance `PublicGuidanceController` notices lane via `PublicGatewayNoticesBffController` | PF-W2 | Public Notices & Bulletins archive — browsable PUBLISHED/in-window/audience=public notices across categories (PUBLIC_HEALTH_CAMPAIGN, SAFETY_RECALL, DISASTER_ALERT, REGULATORY_NOTICE, PROCUREMENT_NOTICE, SERVICE_STATUS); allow-listed fields (no authoring internals, no PII); unknown category coerced to unfiltered (no error oracle). SAFETY_RECALL + REGULATORY_NOTICE require a named approver before PUBLISHED (`AdvisoryAdminService`) — the legal guardrail; no path from internal disciplinary/case data to this lane. Covered by the `GET /internal/v1/public/gateway/**` permitAll (no new SecurityConfig line / Envoy route) |
