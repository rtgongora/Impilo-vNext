...
| **TPL-2** | 🟠 High | OPA orphaned; ABAC corpus unmounted/untested (§4) | **PARTIAL (Phase 3, `792ee620f`).** Correction: the orphaned `infra/opa/impilo/*.rego` does **not compile** under OPA 0.68 (16 parse errors — bare `in`/`startswith` without `rego.v1`/`future.keywords`), so it could not be "promoted"; a clean **`impilo.authz` module** was authored instead (`infra/opa/authz/`, `opa test` 7/7) + `OpaDecisionClient` + a PolicyEngine **SHADOW** seam (`opa-mode` OFF default, never authoritative). **Remaining:** DB-`policy_rules`→OPA-bundle pipeline (full RBAC/ABAC), shadow→enforce cutover, and fixing/retiring the 7 broken doctrine modules. |
