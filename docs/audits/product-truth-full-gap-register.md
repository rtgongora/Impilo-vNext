...
- **OPA-as-PDP — SHADOW (not yet ENFORCE).** `shadowCompareOpa` (`PolicyEngine.java:615`) logs divergence; **Java stays authoritative** (`:227`). **CORRECTION (Phase 7, `bdd7f9aa0`):** the doctrine corpus `infra/opa/impilo/*.rego` **does compile** under OPA 0.68 (`opa check --strict` exits 0 — the earlier "doesn't compile" note was stale); it is now **opa-tested** (31 cases — khuluma/organisation/vashandi) and **mounted** at `/policies/impilo`. The doctrine "OPA decides policy" is still **not achieved** — strangler remains SHADOW only (input-schema/bundle/equivalence-harness + SHADOW→ENFORCE are the remaining, live-gated track; see deferred-for-live-testing.md). Maturity: **REAL_CODE (corpus tested+mounted) · enforcement SHADOW**.
...
| **G-KH-02** | ✅ **W5 built** | **Channels/communities/broadcast built** (`3e7ba29c5`). V004 adds conversation `scope_type`+`scope_ref` + discovery index (runtime-proven V001→V004 PG16); `ChannelService` create (scoped CHANNEL/COMMUNITY, creator=OWNER), discover-by-scope, idempotent join/leave (re-join reactivates), broadcast (OWNER/MODERATOR-only, members are recipients) w/ last-message denorm + outbox events; `ChannelController` full surface. Tested 6/6. | ✅ W5 done | W6–W8 remain |
| **G-KH-03** | ✅ **W6 built** | **External adapter abstraction built** (`526dcb4b3`). V005 `khuluma_channel_adapter` + `khuluma_delivery_attempt` (runtime-proven V001→V005 PG16); `DeliveryService.dispatch` records an auditable attempt per channel — IN_APP→DELIVERED (native), external→SENT only when CONFIGURED else **SKIPPED_NOT_CONFIGURED** (never a fake send); configureAdapter + DeliveryController. Tested 6/6 incl. honesty property. | ✅ W6 done | wire real providers when available (flip adapter→CONFIGURED) |
| **G-KH-04** | ✅ **W7 built** | **Secure WS push + Vashandi on-call built** (`25d15b42d`). `WsTokenValidator` + hardened `RealtimeHandshakeInterceptor`: **closed the WS identity-spoofing hole** (unverified actorId/tenantId query params no longer trusted — Envoy headers or a verified JWT only). V006 `duty_status` (OFF_DUTY/ON_DUTY/ON_CALL, runtime-proven V001→V006) + `OnCallService` setDuty/roster + controller. Tested 12/12 (incl. the spoof-rejection proof). | ✅ W7 done | swap the 4–5s poll for the secured WS in clients |
| **G-KH-05** | 🟢 **Decision core built** | **Recipient-aware safe disclosure built** (`81b5ea056`). `SafeDisclosureService` (khuluma) — **fail-safe to redact**: full content released ONLY to the subject (SELF) or a delegate (guardian/caregiver/CHW) with active consent AND a non-restricted category; every other path (no relationship / no consent / restricted category / null inputs) → content-free safe summary. Relationship resolved upstream from mvumo `DelegationService.resolve`. Tested 8/8 incl. combinatorial no-leak proof. **Remaining:** wire the send path (mvumo resolve + consent check + apply to the khuluma/notification dispatch) | decision core proven; send-path wiring pending | wire mvumo-resolve + consent into the dispatch path |
| **G-KH-06** | 🟢 **Built** | **Nompilo handoff lifecycle + feedback routing built** (`fc329bef2`/`fcf61b7ff`). guidance-service: `nompilo_handoff` (V003, runtime-proven) + `NompiloHandoffService` state machine QUEUED→ACCEPTED→(ESCALATED)→CLOSED, emits lifecycle events (canonical `core.nompilo.handoff.requested`) via outbox; `NompiloHandoffController` request/accept/escalate/close/cancel/queue. khuluma `FeedbackRoutingService` classifies quality/safety feedback → exact Rito quality-signal body (closes G-RT-03 loop). Tested: handoff 6/6 + routing 5/5. **Remaining:** BFF→guidance wiring (replace the stub) + mobile handoff client + the Rito HTTP post at the call site | lifecycle + routing proven; cross-service wiring pending | wire BFF requestNompiloHandoff → guidance; mobile client; rito post |
...
| **G-FU-02** | 🟡 Med · **PARTIAL (gate done; consumer pending)** | **Tshepo training-gate — graduated levels implemented.** PO resolved PO-20260629-01: **levels of permission** (ADVISORY/SOFT/HARD). `FundoTrainingGateService.evaluate` now returns per-requirement `enforcementLevel` (supplied as `CODE:LEVEL`, default ADVISORY) + graduated `decision` (ALLOW/ADVISE/CONDITIONAL/BLOCK) + `blocking` set; boundary preserved (`decisionAuthority: tshepo-authz`). Tested 8/8. **Remaining:** the vashandi→fundo consumer that acts on `decision` at check-in/workspace-entry + the governed workspace/role→course mapping (a plain build, no PO input). | gate: graduated levels live (FundoTrainingGateServiceTest 8/8) | build the consumer + requirement mapping to close the enforcement half |
| **G-FU-03** | 🟢 **CLOSED — native consumer (57375956d)** | **Varapi native CPD egress consumer QUEUED** — `certificate.issued.v1` carries full CPD-candidate shape + egress spec doc, but the varapi-side consumer that lands native completions into the CPD candidate flow is not wired (correctly does NOT duplicate Varapi's ledger) | partial | wire varapi listener (provider/workforce lane) |
...
| ID | Sev | Finding | Status | Decision |
|----|-----|---------|--------|----------|
| **G-PS-01** | ✅ **CLOSED** | **Enforced end-to-end.** Gateway RBAC (V020 `cc5e43841`) + **in-service citizen-own binding** (`requireOwnReportForCitizen`; tested 403/200/own-list) + **obligation consumption** (`f320bb77a`/`f7917cf90`): list honors `x-max-scope=FACILITY_SCOPE` → own-facility only (unknown facility = deny/empty); read+list shaped via `JsonRepresentationShaper` for `suppressFields`/`piiAccess` (fail-closed). Reuses the PDP's existing visibility obligations (not a new roles field) — see [[visibility-obligations-are-the-masking-seam]]. Tested: `facility_scope_obligation_narrows_staff_list_to_own_facility`, `suppress_fields_obligation_redacts_pii_on_read` (suite 8/8) | ✅ closed | per-rule visibility overlays in policy are an authoring follow-up |
| **G-PS-02** | 🟢 **CLOSED — consumer test (5e9b2b87e)** | **Surveillance `PatientSafetySignalConsumer` has no dedicated test** — signal-feed path unverified (service suite 25/25 green; consumer logic correct on read but uncovered) | gap | add a consumer unit/slice test |
...
| ID | Sev | Finding | Status | Decision |
|----|-----|---------|--------|----------|
| **G-RT-01** | ✅ **CLOSED** | **§7 permission model enforced at ext_authz** (V021 `f10fa85f1`): roles reconciled, case-create/quality-ops/reads gated, close-restriction via DENY-wins, REGULATOR added. Citizen case-read **bound + restored** (`requireReadable`/`isOwnCase`; sub-reads inherit via get(); list own-only; tested `client_can_only_read_own_case`; rule reactivated V025 `59febcce0`). **§3 sensitive-category identity REDACTION + facility-scope now enforced** via obligation consumption (`b22c10248`): all 6 case reads shaped through `JsonRepresentationShaper` (`suppressFields`/`piiAccess`); list honors `x-max-scope=FACILITY_SCOPE`. Reuses the PDP visibility obligations — see [[visibility-obligations-are-the-masking-seam]]. Tested `facility_scope...`/`suppress_fields_obligation_redacts_case_identity_on_read` (rito 8/8) | ✅ closed | per-rule sensitive-case visibility overlays = policy-authoring follow-up |
...
| ID | Sev | Finding | Status | Decision |
|----|-----|---------|--------|----------|
| **G-CT-01** | ✅ **CLOSED (patient lane built)** | **Patient lane built end-to-end.** Backend (`6f2578616`/`d4e5d7bf3`): BFF read-composition `GET /internal/v1/mobile/citizen/visit/{transactionId}/status` (PCT journey) + `.../inpatient/{admissionRef}/status` (inpatient); `PatientLaneService` degrades honestly (unknown/outage → available:false, never 500); `PatientMessageCatalog` typed/plain-language/i18n-ready. Web screens (`4927ec0f9`): `/citizen/visit/[transactionId]` + `/citizen/inpatient/[admissionRef]` (live refresh, honest loading/error/unavailable, no fabricated data), registered in the route registry. Tested: PatientLaneServiceTest 6/6 (incl. persona journey ARRIVED→…→DISCHARGED) + controller 2/2 + vitest 3/3; tsc-clean. **Remaining (refinements):** queue-position/telemedicine-status composition + mobile-app parity | ✅ patient lane surfaced + tested (backend + web) | refine queue/telemed composition; mobile parity |
| **G-CT-02** | 🟢 **CLOSED — structured note + no-closure-without-audit** | **Telemedicine loop-closure lighter than the Addendum-2 mandate.** Stage-6 structured response IS real (V020 `structured_response` diagnosis/actionPlan/redFlags/followUp); Stage-7 `/complete` emits the value event but the **structured referrer Completion Note** (actions-taken / patient-outcome / closure-narrative, "no closure without audit") is thin. The 6 modes route via RTC-gateway session provider; async/chat/MDT-board depth varies | partial | structured completion-note + mode-depth |
...
|----|-----|------------------|-----------|----------------|
| **SYS-1** | 🟡 **Enforced via DB-rule PDP; OPA corpus SHADOW** | **Per-domain policy IS enforced** through the live Java DB-rule PolicyEngine (Phase 1: V019–V025 + in-service guards, all Flyway-proven) — the original "scaffold/shadow only" overclaim is closed for enforcement. The OPA doctrine track: corpus `infra/opa/impilo/*.rego` **compiles** under 0.68 (stale "doesn't compile" note corrected), now **opa-tested (31 cases)** + **mounted** at `/policies/impilo` for SHADOW (`bdd7f9aa0`); Java stays authoritative pending the live SHADOW→ENFORCE cutover. | CZO OPA-scaffold; Provider **G-PX-01**; OROS **G-OR-01** | enforcement live via DB-rule PDP; OPA "decides policy" cutover is the remaining live-gated track (deferred-for-live-testing.md) |
| **SYS-2** | ✅ **CLOSED** | **Product-truth now capability-grained + runtime-proof-aware.** (1) `probeEvidence` wired (`e26f37d11`): `reports/product/probe-evidence.json` → `classifyMaturity` lifts proven services to REAL_PROVEN (metric moved 0→4); test enforces "never REAL_PROVEN without evidence". (2) `reports/product/capability-matrix.json` (`e3b4a64e9`): joins backend routes×frontend surfaces×BFF×contract ops into 3681 capabilities with per-capability disposition (real-proven 207 / real 3139 / partial 141 / fixture 194) — in-service additions are now visible + fixtures pinpointed. Tools `node --test` 16/16. | `reports/product/{probe-evidence,capability-matrix}.json` | in-service feature additions now move the metric; REAL_PROVEN gated on real test runs |
