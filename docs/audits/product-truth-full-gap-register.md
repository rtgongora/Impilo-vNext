...
| G002 | ~~Offline capability token **JWS signature never verified**~~ **✅ FIXED (B2, `fefe6422`):** local Ed25519 JWS verification against JWKS (alg-allowlist + kid + signature + exp/nbf) before any claim is trusted; registry checks follow; audited. `CapabilityTokenJwsVerifierTest` 6/6 + service 16/16 (REAL_PROVEN). | `tshepo-offline-service/.../CapabilityTokenJwsVerifier.java`, `CapabilityTokenService.java` | crypto | done | B2 ✅ |
