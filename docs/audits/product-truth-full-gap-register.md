...
| G059 | **Signing client calls a non-existent keys-service endpoint — breaks TWO live issuance paths.** tshepo-offline `KeysServiceClient.signPayload` POSTs `/v1/sign/jws` `{payload,keyId,algorithm}` and reads `.jws`; keys-service only exposes `/v1/sign` taking `{tenantId,payload,jwsCompact}` returning `.signature`, signing by the tenant's active key (keyId ignored, no tenantId sent) → **404 at runtime**. Wired into **both** `CapabilityTokenService.issueToken:108` (capability-token issuance) **and** `OfflinePackService:109` (offline-pack generation) — both broken. Both pass a `SIGNING_KEY_ID` purpose-ish constant, which maps onto the B0-lite `KeyPurpose` model: the unified fix signs by `{tenantId,payload,purpose}` via `getActiveKeyForPurpose` (threading fix-a, OFFLINE_CAPABILITY / offline-pack purpose). Hidden by mocked tests. | `tshepo-offline-service/.../client/KeysServiceClient.java:35,40`; `.../core/CapabilityTokenService.java:108`; `.../core/OfflinePackService.java:109`; `tshepo-keys-service/.../api/SigningController.java` | contract-mismatch / runtime-break | fix (unify on tenant+purpose signing; runtime-prove) | a / B-core hardening |
