...
| ID | Sev | Finding | Status | Decision |
|----|-----|---------|--------|----------|
| **G-CT-01** | 🟡 **Backend built (screens pending)** | **Patient lane now has real backend surfaces + message catalog** (`6f2578616`/`d4e5d7bf3`). BFF read-composition: `GET /internal/v1/mobile/citizen/visit/{transactionId}/status` (composes PCT journey) + `.../inpatient/{admissionRef}/status` (composes inpatient admission); `PatientLaneService` degrades honestly (unknown id / downstream outage → available:false, never 500); `PatientMessageCatalog` = typed, plain-language, i18n-ready (stable messageKeys) for every JourneyState + inpatient status → person-journey stage. Tested: PatientLaneServiceTest 6/6 (incl. full persona journey ARRIVED→…→DISCHARGED) + controller 2/2. **Remaining:** the citizen **web/mobile screens** (need the one-ui-shell/mobile toolchain) + the queue-position/telemedicine-status composition refinements | backend surfaced + tested; screens pending | build citizen visit/inpatient screens on the new endpoints |
