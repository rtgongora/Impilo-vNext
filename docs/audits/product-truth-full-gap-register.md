...
| G003 | DAGS permit-token HMAC key defaulted to literal **"change-me-in-prod"** — forgeable if env unset. **PARTIALLY FIXED (B3, folded from `intake/b3-dags-permit-key`):** service now **fails closed** on a blank/default signing key; requester bound via full **SHA-256** (not 32-bit hashCode); full-length HMAC-SHA256. Requester is server-side (`approve()` reads the persisted `AccessRequestEntity.requesterId`, not client input). **Remaining B3 scope:** bind resourceType/resourceId/purpose/expiry into the signed payload; replay marker; structured audit (grant/deny/invalid-sig/expired/wrong-requester/wrong-resource); + a verify path (see [[G056]]). | `data-access-governance-service/.../EnforcementService.java` | hardcoded/crypto | partial; extend + verify | B3 |
| G056 | **DAGS permit token signature is never verified by any consumer** — `EnforcementService` issues a signed `permit-token:v1:...` stored on the request, but no enforcement path parses/validates it, so the signature is decorative (a forged or tampered token is never rejected because nothing checks one). | `data-access-governance-service/.../core/EnforcementService.java` (no verify method); no enforcement consumer | unverified-signature | fix (add verifyPermitToken + enforcement check) or mark deferred | B3 |
