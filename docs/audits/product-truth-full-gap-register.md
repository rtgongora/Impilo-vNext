...
| G059 | **Capability-token signing calls a non-existent keys-service endpoint** — tshepo-offline `KeysServiceClient` POSTs `/v1/sign/jws` with `{payload,keyId,algorithm}`, but keys-service only exposes `/v1/sign` taking `{tenantId,payload,jwsCompact}` → **404 at runtime; capability-token issuance is broken**. Hidden by mocked tests (found wiring fix-a; a textbook case for runtime/integration proof) | `tshepo-offline-service/.../client/KeysServiceClient.java:40`; `tshepo-keys-service/.../api/SigningController.java:29` | contract-mismatch | fix (align contract + thread purpose) | a / B-core hardening |
