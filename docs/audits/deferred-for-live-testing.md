...
| **OPA-as-PDP ENFORCE** (`SYS-1`, Phase 7) | **Foundation + Stage A largely done (sandbox-proven):** doctrine corpus compiles + opa-tested (31) + mounted (`bdd7f9aa0`). **A1** condition-eval rego mirror of `evaluateConditions` (`04b0ffd15`, 21/21). **A2** input contract documented (the Java `AuthzInternalRequest` already carries it). **A4** `policy_decision.rego` mirror of Step-4 (`94947533e`, 11/11). **A3** `PolicyRuleBundleBuilder` + a **real Java→OPA round-trip** (`525066545`) proving the Java-built bundle drives OPA to the same verdict. `opa test` 69/69, `--strict` clean | **Remaining A5:** a full equivalence harness running the *actual* Java `PolicyEngine` side-by-side with OPA across a generated corpus (nightly) — the round-trip *mechanism* is proven; instantiating the full PolicyEngine dep-graph is the extension. **Stage B (live-gated):** wire the SHADOW call (B1), bundle delivery runtime (B2), SHADOW on real traffic→divergence 0 (B3), the ENFORCE code path (B4), per-tenant canary→cutover (B5) | OPA authoritative (fail-closed), divergence→0, validated live before cutover |
