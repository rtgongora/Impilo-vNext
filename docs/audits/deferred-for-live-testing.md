...
| **OPA-as-PDP ENFORCE** (`SYS-1`, Phase 7) | `shadowCompareOpa` logs divergence; Java authoritative; authz gate (`opa test` 7/7). **Phase-7 foundation done (`bdd7f9aa0`):** the doctrine corpus **compiles** under 0.68 (the "doesn't compile" note was stale), is now **opa-tested** (31 cases across khuluma/organisation/vashandi) and **mounted** at `/policies/impilo` for SHADOW | The remaining Phase-7 migration: expand the OPA **input schema**, the `policy_rule`→OPA **bundle pipeline**, condition-eval rego mirrors, a **Java↔OPA equivalence harness** (staging-replay), then **SHADOW→ENFORCE** canary per tenant in a live cluster | OPA is authoritative (fail-closed) with divergence→0, validated live before cutover |
