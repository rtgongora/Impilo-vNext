...
| `NCZ-DEC-016` | **Are the existing speciality registers statutory registers, or qualifications?** | varapi already seeds NCZ_GENERAL, NCZ_MENTAL_HEALTH, NCZ_MIDWIFE and NCZ_SPECIALIST. The brief names four different registers — Student, Provisional, Main and Nurse-Led Health Institutions — which measure standing rather than speciality. Both readings are defensible and the original seed mixed them (MDPCZ has a PROVISIONAL register alongside its speciality ones), so the platform will not decide it by inference: what a register is called on a certificate is a statutory matter | `professional_registers.register_axis` — STANDING or QUALIFICATION, left NULL on every pre-existing row rather than guessed | Nothing operationally. Both sets coexist and the brief's four are seeded and usable. What is blocked is retiring or re-labelling the speciality registers, and how a nurse holding several qualifications on the Main Register is described. |
