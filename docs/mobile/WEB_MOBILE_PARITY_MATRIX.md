...
| Theatre | Partial parity | TheatreQueueScreen + TheatreCaseScreen over BFF `/internal/v1/theatre/**` (queue, readiness, book/start, note, PACU, safety, death). Wave M2 bedside OR panels on case screen: site/side, specimens custody, counts, implants, blood, emergency consent exception, return-to-theatre — list GET failures surface unavailable (not empty). Gaps vs web: no readiness board, surgical referrals, commodities remove/revise/recall, obstetric cascade, anaesthesia chart, discharge summary, or Maestro theatre flow. TheatreProcedureScreen episode wizard remains. |
