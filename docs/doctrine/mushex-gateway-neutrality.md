...
| Observability / audit / reconciliation / operations surface coherence. **Phase 8 implemented in-repo (8A–8F).** Outbox catalogue remains canonical and is now enforced by routing-convention tests (MusheX + COSTA). Finance-plane runbook is published. Audit list now supports aggregate filters (`aggregateType`, `aggregateId`) in BFF and `/admin/audit`. Reconciliation now includes a triple-source read join endpoint/UI panel (`invoice ⇄ intent ⇄ settlement`) for encounter-scoped checks. Topic consumer map is published at [`phase-8-topic-consumer-map.md`](../audits/phase-8-topic-consumer-map.md). | **G-13** | Phase 8 (implemented, repository-owned scope) |
| Retirement-readiness ledger + telemetry-signals catalogue. **Phase 7 follow-on implemented:** ledger and signal catalogue remain canonical, with added query recipes (`docs/retirement/telemetry-query-recipes.md`), parity evidence (`docs/audits/phase-7-retirement-parity-audit.md`), legacy wallet-route inbound counter (`impilo.legacy.route.requests` in Experience BFF), and CI guard workflow blocking net-new files under deprecated folders. Retirement itself remains telemetry-gated and explicit. | **G-12** | Phase 7 (7D/7E/7F done; 7B/7C dashboard wiring operational) |
| Claims / coverage / remittances / subsidies surface coherence. **Phase 6 implemented for Slices 6A–6E.** In addition to the already-landed 6A–6D reads and UI coherence changes, Slice 6E now surfaces subsidies as first-class read-only rows via `GET /internal/v1/finance/billing-workspace/lifecycle/subsidies?encounterId=...` (backed by COSTA lifecycle), shown on `/finance/costa` and merged into `/finance/costa/encounter/[encounterId]` timeline. No subsidy write action is introduced. Full audit in [`docs/audits/phase-6-claims-coverage-remittance-subsidy-audit.md`](../audits/phase-6-claims-coverage-remittance-subsidy-audit.md). | **G-11** | Phase 6 (Slices 6A–6E complete) |
