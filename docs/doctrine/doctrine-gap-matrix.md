...
| GW-05 | Persistent Emergency Help (§7) | PARTIAL | dispatch workflow BUILT; W1 landed: persistent `EmergencyHelpButton` (text+icon, accessible) on public AND authenticated chrome → `/welcome/emergency`, no identity/coverage/payment checks in its path | Anonymous intake lane + abuse controls + callback-before-dispatch (PD-3) in W2; mobile persistent control | `ui/one-ui-shell/src/components/public/EmergencyHelpButton.tsx`, `services/daidzai-service/.../EmergencyController.java` |
