...
## IATG Real-Life Journeys (RJ-1…RJ-6) — MERGED TO ANCHOR 2026-07-07 (user-authorized "Merge")
Stream `fable/iatg-realjourney` @ `a5326f477`, accumulated off E3.2 base `ee059dd47` (one branch, six waves, one merge). **Fast-forward anchor `8efda86fc` → `a5326f477`, pushed** — the anchor was the exact merge-base, so this advance also lands the previously-unmerged **E3.2 harness-contract fixes** (`ee059dd47`). 56 files, +3766/−59. **`services/tshepo-service/**` = 0** (invariant held across all 15 commits); no org-registry cutover / phase-2c / write-freeze / mirror-producer files touched. Completes the six real-life browser journeys A–F so a real person can enter real data, hand off to the correct next actor, and reach a governed outcome in the product (Section-12 standard: not "the code exists" but "a real person can use it").
- **RJ-1 Journey A (durable EXECUTED)** — WGV `PlatformOriginService.execute()` stamps `status=EXECUTED` + new `getAction()`; controller `GET /actions/{id}`; BFF `PlatformOriginGovernanceClient.getAction` + proxy; UI `useAction`/`PlatformActionState` so the console shows the true terminal state after a browser refresh (no optimistic in-memory state).
- **RJ-2 Journey D entry** — "Request Provider Access" surfaced for Health-ID citizens (life-zone sidebar entry + quick-access rail + heading); not a dead link, not admin-only.
- **RJ-3 Journey D (durable request)** — new varapi `provider-access-request` aggregate (`V021`, `ProviderAccessRequestEntity/Service/Controller`, enums `ProviderAccessRequestType{NEW_PROVIDER,ORG_INVITATION,COUNCIL_NUMBER,EC_NUMBER,HAVE_PROVIDER_ID,RECOVER}`/`Status{…}`, `DUPLICATE_SUSPECTED` recover-not-reissue guard); BFF submit/status reads (`/access-request`, `/status[/{publicId}]`), org-invitation now durable; UI new/invite lanes + `/citizen/provider-claim/status` page. Every submission persists a `publicId` with a status + named next actor.
- **RJ-4 Journey E (Facility Mode)** — BFF facility-admin appointments read (`GET /api/v1/facility-claim/appointments`); UI `useFacilityMode` (`administersFacility`/`pendingAppointments`), `FacilityModeDashboard`, `/facility/[id]/mode`; enter-mode trigger. Eligibility backed by a real per-facility appointment read, not a global role.
- **RJ-5 Journey F (adjudication producers)** — WGV WS-D employment-CONFLICT producer (`V010`, `WgvEmploymentConflictCaseEntity/Service`, `WgvWorkflowServiceClient` def `…-0001`, gate `impilo.governance.adjudication.enabled`) + varapi WS-F provider-claim-CONFLICT producer (`ProviderClaimAdjudicationService` REQUIRES_NEW, `ProviderWorkflowServiceClient` def `…-0002`, gate `impilo.varapi.adjudication.enabled` default-off → claim path byte-identical when off; resolve write-back documented as doctrine follow-up).
- **RJ-6 seed/harness/docs** — `scripts/seed/iatg-realjourney-seed.sh`: 21-scenario idempotent seeder (4 realm principals documented + 17 seeded via reachable REST; honest `SKIPPED`-with-reason; resolves the **canonical** `facility_uuid` via search→id→`/configuration`, not the search `facilityUid`); harness read-back steps (1 durable `GET /actions/{id}==EXECUTED`, 10 access-request submit→read-back-by-publicId, 11 facility-mode appointments read); `docs/runbooks/iatg-real-life-journeys-runbook.md` (A–F superset + Section-11 acceptance); `docs/coordination/iatg-realjourney-contract-verification.md` (C/D/E/F payload→BFF DTO→downstream with file:line + failure-code matrix).
- **DEFECT found + fixed (Journey E blocker):** `administersFacility()` gated on `approvalState === "APPROVED"`, a literal tuso **never emits** (states are `PENDING/ACTIVE/REJECTED/REVOKED`; approve() sets `ACTIVE`) — Facility Mode could never activate for a legitimately-approved admin. Corrected to `ACTIVE` + regression guard test; the prior test had encoded the bug with `"APPROVED"` fixtures (rewritten).
- **Gates (coordinator-run, real exit codes):** varapi `mvn test` full module 0 (`ProviderClaimAdjudicationServiceTest` 3/3 + `ProviderBootstrapServiceTest` new-ctor + suite); UI `type-check` 0, `vitest` facility-mode + facility-claim 12/12; `shellcheck` seed + harness 0, both `--dry-run` 0 (seed manifest 4 principals + 17 seeded + 0 failed / 21).

**IATG VERDICT STILL PENDING — `IATG_NOT_READY` off code+gates alone; ZERO browser/runtime evidence exists.** `IATG_REAL_LIFE_JOURNEYS_PROVEN` (Section-11: seed exit 0 + harness exit 0 + browser A–F + 20 acceptance conditions) is assigned by the coordinating session ONLY after the user runs `iatg-real-life-journeys-runbook.md` on the preview VM (deploy at `a5326f477` + 21-scenario seed + read-back harness + browser A–F) and the returned evidence is adjudicated. This container is egress-blocked from `41.57.127.235` (403) and cannot run it. NOT relabeled "proven end-to-end".
