...
**Net**: the 96-ungated-module gap is closed on the VM gate — every reactor module's unit tests now run on every VM gate invocation. **WS-P6-B oros still needs its one-time green VM run** (`bash scripts/test/run-backend-checks.sh` on the preview VM — dcm4che reachable there → the `oros-imaging-tests` + `backend-reactor-tests` gates both execute) to convert the CONDITIONAL status to CLOSED with a real exit code + surefire summary. **Phase 3** (burn down any future quarantine additions) is a no-op today — the list is empty.
