...
| Facility Mode cockpit/setup/units/control-tower | Partial | N/A | Partial | Web surfaces live (`/facility/[id]/*`). **Mobile (2026-07-01):** provider-app now has a **Control Tower** tab (`ControlTowerScreen` + `controlTowerService` → live `/internal/v1/facility-mode/control-tower/{aggregate,alerts,acknowledge}`) — GAP-19 partially closed. Also on mobile now: **Setup** tab (`FacilitySetupScreen` + `facilitySetupService` → live `/setup`, `/units`, `/service-points`, `/setup/steps` — readiness checklist with per-step advance/reopen + go-live for all wizard steps — departments,
service-points, queues, workflows, workforce, OROS-routing, comms, learning — via `/setup/steps`,
surfacing TUSO's honest go-live rejection; create departments/service-points) and **Regulators** tab.
Remaining mobile depth: richer per-step config editors (queue/workflow definitions themselves). |
| Facility↔regulator (multi-council) | Partial | N/A | Partial | Web `/facility/[id]/regulators` live. **Mobile (2026-07-01):** provider-app **Regulators** tab (`FacilityRegulatorsScreen` + `regulatorService` → live `/internal/v1/facility-mode/{id}/regulators` list + link-council) — facility picked from the control-tower aggregate. GAP-19 partially closed. |
