...
public class OutboxEventComplianceTest {
    ...
    void coveragePlanOutboxCompliant() throws Exception {
        ...
        List<OutboxEventEntity> recent = outboxRepository.findTop100ByPublishedAtIsNullOrderByCreatedAtAsc();
        OutboxEventEntity latest = recent.get(recent.size() - 1);
        ...
        assertThat(payload.has("event_type")).isTrue();
        assertThat(payload.get("event_type").asText()).startsWith("impilo.coverage.");
        assertThat(payload.get("event_type").asText()).endsWith(".v1");
        assertThat(payload.has("schema_version")).isTrue();
    ...
    void remittanceOutboxCompliant() throws Exception {
        ...
        JsonNode payload = MAPPER.readTree(latest.getPayloadJson());
        assertThat(payload.get("meta").get("partition_key").asText()).isNotBlank();
    }
    ...
    void eligibilityOutboxCompliant() throws Exception {
        ...
        mockMvc.perform(post("/internal/v1/coverage/eligibility")
                        .header("X-Tenant-ID", TENANT_ID)
                        .header("X-Pod-ID", "national-spine")
        ...
        assertThat(latest.getEventType()).isEqualTo("impilo.coverage.eligibility.created.v1");
        assertThat(latest.getSubjectType()).isEqualTo("Eligibility");

        JsonNode payload = MAPPER.readTree(latest.getPayloadJson());
        assertThat(payload.has("data")).isTrue();
        assertThat(payload.get("data").has("meta")).isTrue();
        // Decision evidence in event payload meta
        JsonNode dataMeta = payload.get("data").get("meta");
        assertThat(dataMeta.has("decision")).isTrue();
        assertThat(dataMeta.get("decision").has("evidence")).isTrue();
    }
