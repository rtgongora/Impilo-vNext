...
@Component
public class GeminiLlmProvider extends HttpLlmProviderSupport {
    private final LlmProperties properties;

    public GeminiLlmProvider(RestTemplate llmRestTemplate, ObjectMapper objectMapper, LlmProperties properties) {
    ...
    @Override
    public boolean enabled() { return properties.getGemini().enabled(); }

    @Override
    public String model() { return properties.getGemini().model(); }

    @Override
    public int priority() { return properties.getGemini().priority(); }
    ...
    public LlmResponse invoke(LlmRequest request) {
        return callOpenAiCompatible(name(), model(), properties.getGemini().baseUrl(), properties.getGemini().apiKey(), request);
    }
}
