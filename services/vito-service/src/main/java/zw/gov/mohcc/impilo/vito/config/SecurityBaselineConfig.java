...
/**
 * Wave 14 security hardening configuration for VITO.
 */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("vito", new AdminAuditOutboxSink(dataSource, "vito"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_VITO_");
    }
