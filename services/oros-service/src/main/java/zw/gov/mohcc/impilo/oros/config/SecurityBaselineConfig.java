...
/**
 * Wave 14 security hardening configuration for OROS.
 */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("oros", new AdminAuditOutboxSink(dataSource, "oros"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_OROS_");
    }
