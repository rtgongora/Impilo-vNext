package zw.gov.mohcc.impilo.telemonitoring.config;
...
/** Security baseline (admin audit + rate limit) — house idiom mirrored from participation-service. */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("telemonitoring", new AdminAuditOutboxSink(dataSource, "telemonitoring"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_TELEMONITORING_");
    }
