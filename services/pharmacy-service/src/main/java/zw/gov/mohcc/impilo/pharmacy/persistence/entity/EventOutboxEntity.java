...
import zw.gov.mohcc.impilo.pharmacy.events.PharmacyEventTypes;
 ...
 *
 * <p>{@code payloadJson} holds the domain payload alone, never a nested envelope. That is
 * load-bearing here: the legacy topic {@code pharmacy.dispense.complete} feeds
 * {@code InventoryEventConsumer.handleDispenseComplete}, which reads {@code tenantId} at the
 * JSON top level and skips in silence rather than throwing if it is not there.</p>
 *
 * <p>The v1.1 companion columns default in {@link #onCreate()} rather than at each of the
 * thirteen call sites that construct this entity. All of them already set the tenant, which
 * is the one field that cannot be defaulted truthfully.</p>
 */
@Entity
@Table(name = "rx_event_outbox")
public class EventOutboxEntity {
    ...
    private static final String PRODUCER = "pharmacy-service";
    ...
    public PharmacyOutboxRow toOutboxRow() {
        return new PharmacyOutboxRow(this);
    }

    /**
     * Exposes both event-type forms. The stored value is legacy ({@code DISPENSE_COMPLETED})
     * and is what legacy topic routing switches on; the v1.1 envelope needs
     * {@code impilo.{service}.{entity}.{action}.v{N}}, and CompanionOutboxPublisher puts
     * {@code eventType()} straight into the envelope.
     */
    public static final class PharmacyOutboxRow implements CompanionOutboxPublisher.OutboxRow {

        private final EventOutboxEntity self;

        PharmacyOutboxRow(EventOutboxEntity self) {
            this.self = self;
        }
        ...
        @Override public String eventType() {
            return PharmacyEventTypes.canonical(self.aggregateType, self.eventType);
        }
    }
