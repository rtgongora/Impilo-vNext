...
/**
 * Security baseline — admin audit outbox, rate limiting, secret provider.
 * Mirrors the Wave 14 baseline applied across registry services.
 */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("org_registry", new AdminAuditOutboxSink(dataSource, "org_registry"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_ORG_REGISTRY_");
    }
