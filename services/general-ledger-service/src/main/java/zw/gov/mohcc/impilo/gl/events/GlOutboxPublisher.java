...
@Component
@Profile("!test")
public class GlOutboxPublisher extends CompanionOutboxPublisher {

    private static final Logger log = LoggerFactory.getLogger(GlOutboxPublisher.class);

    private final EventOutboxRepository outboxRepository;
    private final KafkaTemplate<String, String> kafkaTemplate;

    public GlOutboxPublisher(EventOutboxRepository outboxRepository,
                             KafkaTemplate<String, String> kafkaTemplate,
                             @Value("${gl.v11.emit-mode:#{null}}") String ymlEmitMode) {
        super(new DualEmitPolicy(ymlEmitMode), new EventTopicRegistry("general_ledger"));
        this.outboxRepository = outboxRepository;
        this.kafkaTemplate = kafkaTemplate;
        log.info("GlOutboxPublisher initialized with emit-mode={}", effectiveEmitMode());
    }

    @Scheduled(fixedDelayString = "${gl.outbox.poll-interval-ms:2000}")
    @Transactional
    public void poll() {
        int count = publishPendingEvents();
        if (count > 0) {
            log.info("Published {} general-ledger outbox events", count);
        }
    }
    ...
    protected String resolveLegacyTopic(OutboxRow row) {
        return switch (row.aggregateType()) {
            case "JOURNAL_ENTRY" -> "gl.journal";
            case "FISCAL_PERIOD" -> "gl.period";
            default -> "gl.events";
        };
    }
}
