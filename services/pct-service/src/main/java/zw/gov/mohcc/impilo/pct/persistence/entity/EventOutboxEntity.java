...
import zw.gov.mohcc.impilo.pct.events.PctEventTypes;
...
/**
 * Outbox pattern entity for reliable event publishing to Kafka.
 * Events are written transactionally alongside domain changes, then
 * published asynchronously by a polling publisher that marks them as published.
 *
 * <p>{@code payloadJson} holds the domain payload alone, never a nested envelope — the
 * envelope is built at publish time, which is what lets one row feed the legacy topics (raw)
 * and the v1.1 topic (enveloped) without the two shapes colliding.</p>
 *
 * <p>The v1.1 companion columns default in {@link #onCreate()} rather than at each of the 45
 * construction sites across 17 {@code writeOutbox} helpers. All of them already set the
 * tenant, which is the one field that cannot be defaulted truthfully.</p>
 */
@Entity
@Table(name = "pct_event_outbox")
public class EventOutboxEntity {
    ...
    private static final String PRODUCER = "pct-service";
    ...
    public PctOutboxRow toOutboxRow() {
        return new PctOutboxRow(this);
    }

    /**
     * Exposes both event-type forms. PCT's stored types are legacy and come in two flavours —
     * UPPER_SNAKE ({@code JOURNEY_STATE_CHANGED}) and already-dotted
     * ({@code pct.observation.recorded}) — and {@code routeTopic} switches on exactly those
     * strings. The v1.1 envelope needs {@code impilo.{service}.{entity}.{action}.v{N}}, and
     * CompanionOutboxPublisher puts {@code eventType()} straight into the envelope, so the two
     * cannot be the same value.
     */
    public static final class PctOutboxRow implements CompanionOutboxPublisher.OutboxRow {

        private final EventOutboxEntity self;

        PctOutboxRow(EventOutboxEntity self) {
            this.self = self;
        }
        ...
        @Override public String eventType() {
            return PctEventTypes.canonical(self.aggregateType, self.eventType);
        }
    }
