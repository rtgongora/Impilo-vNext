...
public class EdDiagnosticsService {
    ...
    public Map<String, Object> reconcileCriticalResult(Map<String, Object> body) {
        ...
        // Acknowledge via OROS's own order-scoped ack endpoint (closes the loop, audited in OROS).
        boolean acked = orderId != null && oros.acknowledgeCriticalOrder(orderId, reason);
        ...
        if (link != null) {
            ...
            if (acked) link.setAckedAt(OffsetDateTime.now());
