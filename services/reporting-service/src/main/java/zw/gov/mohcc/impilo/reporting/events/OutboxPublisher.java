...
public class OutboxPublisher {
    ...
    @Scheduled(fixedDelayString = "${reporting.outbox.poll-interval-ms:2000}")
    @Transactional
    ...
    static String routeTopic(String eventType) {
        return switch (eventType) {
            case "REPORT_CREATED" -> "impilo.reporting.report.created.v1";
            case "REPORT_RAN" -> "impilo.reporting.report.ran.v1";
            case "SCHEDULE_CREATED" -> "impilo.reporting.schedule.created.v1";
            default -> "impilo.reporting.events";
        };
    }
}
