...
/**
 * Wave 14 security hardening configuration for MSIKA.
 */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("msika", new AdminAuditOutboxSink(dataSource, "msika"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_MSIKA_");
    }
