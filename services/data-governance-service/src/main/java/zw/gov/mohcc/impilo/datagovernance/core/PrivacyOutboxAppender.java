...
/**
 * Transactional-outbox appender for the privacy-rights sub-domain (data-subject
 * requests, privacy display preferences). Mirrors the outbox contract used by
 * {@code GovernanceService} so every privacy-rights state change reliably emits
 * a domain event for downstream consumers.
 */
@Component
public class PrivacyOutboxAppender {
    ...
    public PrivacyOutboxAppender(OutboxEventRepository outboxRepository, ObjectMapper objectMapper) {
