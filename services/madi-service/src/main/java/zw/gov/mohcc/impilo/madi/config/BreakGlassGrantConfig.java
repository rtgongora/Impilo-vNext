...
/**
 * Wires madi's emergency guards to the tshepo-authz grant-validation seam.
 *
 * <p>The RestTemplate is deliberately its own, not the shared {@link RestTemplateConfig} one. That
 * bean carries a 5s connect / 10s read timeout, which is reasonable for ordinary integration and
 * wrong here: this call sits between a clinician and an uncrossmatched blood release. Fifteen
 * seconds of waiting for a policy service is worse for the patient than a prompt UNREACHABLE, which
 * allows the release and records an ungoverned override for review. Short timeouts are a safety
