...
/**
 * Writes domain events into the Participation transactional outbox. Called inside the same
 * transaction as the state change so events are never lost (relayed by
 * {@link ParticipationOutboxPublisher}).
 */
@Component
public class ParticipationEventEmitter {

    private static final Logger log = LoggerFactory.getLogger(ParticipationEventEmitter.class);

    private final EventOutboxRepository outboxRepository;
    private final ObjectMapper objectMapper;

    public ParticipationEventEmitter(EventOutboxRepository outboxRepository, ObjectMapper objectMapper) {
