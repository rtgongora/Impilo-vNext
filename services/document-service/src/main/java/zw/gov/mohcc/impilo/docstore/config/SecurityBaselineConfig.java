...
/**
 * Wave 14 security hardening configuration for DOCSTORE.
 */
@Configuration
public class SecurityBaselineConfig {
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("docstore", new AdminAuditOutboxSink(dataSource, "docstore"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_DOCSTORE_");
    }
