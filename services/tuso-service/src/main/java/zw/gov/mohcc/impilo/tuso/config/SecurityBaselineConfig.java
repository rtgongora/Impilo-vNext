...
/**
 * Wave 14 security hardening configuration for TUSO.
 */
@Configuration
public class SecurityBaselineConfig {

    // TUSO: 150 requests/min capacity, ~2 tokens/sec refill
    ...
    // presents the same actor, so a suite of more than 150 requests throttles itself.
    // Defaults are this service's own previous values -- runtime behaviour is unchanged.
    @Bean
    public RateLimitGuard rateLimitGuard(
            @Value("${impilo.security.rate-limit.max-tokens:150}") long maxTokens,
    ...
    public AdminAuditEmitter adminAuditEmitter(DataSource dataSource) {
        return new AdminAuditEmitter("tuso", new AdminAuditOutboxSink(dataSource, "tuso"));
    }

    @Bean
    public SecretProvider secretProvider() {
        return new EnvSecretProvider("IMPILO_TUSO_");
    }
