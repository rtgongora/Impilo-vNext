...
public class NhumeClient {
    ...
    public NhumeClient(RestTemplate restTemplate,
                       @Value("${booking.integrations.nhume-base-url}") String baseUrl) {
        this.restTemplate = restTemplate;
        this.baseUrl = IntegrationHttpSupport.trimSlash(baseUrl);
    }

    public Map<String, Object> createDispatchLink(TrustContext ctx, Map<String, Object> payload) {
        return post(ctx, "/v1/internal/dispatch/requests", payload);
    }
    ...
    private Map<String, Object> post(TrustContext ctx, String path, Map<String, Object> payload) {
        try {
            ...
            log.warn("Nhume POST {} failed: {}", path, ex.getMessage());
            return Map.of();
        }
    }
