...
public class LearningClient {
    ...
    public LearningClient(RestTemplate restTemplate,
                          @Value("${booking.integrations.learning-base-url}") String baseUrl) {
        this.restTemplate = restTemplate;
        this.baseUrl = IntegrationHttpSupport.trimSlash(baseUrl);
    }

    public Map<String, Object> createTrainingSessionLink(TrustContext ctx, Map<String, Object> payload) {
        return post(ctx, "/v1/internal/sessions", payload);
    }
    ...
    private Map<String, Object> post(TrustContext ctx, String path, Map<String, Object> payload) {
        try {
            ...
            log.warn("Learning POST {} failed: {}", path, ex.getMessage());
            return Map.of();
        }
    }
