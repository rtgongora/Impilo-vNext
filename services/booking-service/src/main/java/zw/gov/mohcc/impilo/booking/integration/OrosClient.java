...
public class OrosClient {
    ...
    public OrosClient(RestTemplate restTemplate,
                      @Value("${booking.integrations.oros-base-url}") String baseUrl) {
        this.restTemplate = restTemplate;
        this.baseUrl = IntegrationHttpSupport.trimSlash(baseUrl);
    }

    public Map<String, Object> createLabOrderLink(TrustContext ctx, Map<String, Object> payload) {
        return post(ctx, "/v1/internal/orders", payload);
    }
    ...
    private Map<String, Object> post(TrustContext ctx, String path, Map<String, Object> payload) {
        try {
            ...
            log.warn("OROS POST {} failed: {}", path, ex.getMessage());
            return Map.of();
        }
    }
