...
public class CoverageClient {
    ...
    public CoverageClient(RestTemplate restTemplate,
                          @Value("${booking.integrations.coverage-base-url}") String baseUrl) {
        this.restTemplate = restTemplate;
        this.baseUrl = IntegrationHttpSupport.trimSlash(baseUrl);
    }

    public Map<String, Object> checkCoverage(TrustContext ctx, Map<String, Object> payload) {
        return post(ctx, "/v1/internal/coverage/check", payload);
    }
    ...
    private Map<String, Object> post(TrustContext ctx, String path, Map<String, Object> payload) {
        try {
            ...
            log.warn("Coverage POST {} failed: {}", path, ex.getMessage());
