 ...
 *
 * Populated by {@link TrustContextFilter} at the start of each request
 * and cleared at the end. Accessible from any layer (controller, service,
 * repository) without passing the context explicitly.
 */
public final class TrustContextHolder {
    ...
    public static TrustContext require() {
        ...
        if (ctx == null) {
            throw new IllegalStateException("TrustContext not set — request did not pass through TrustContextFilter");
        }
        return ctx;
    }
